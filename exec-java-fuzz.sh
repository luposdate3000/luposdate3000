...
do
	...
	(
		...
		while true
		do
			java -javaagent:dependencies/jacocoagent.jar=destfile=jacoco.${db}.exec -cp "$(pwd)/${output2}/distributions/luposdate3000/lib/*" MainKt "db${db}" $size 0
		done
	)  >> log/db$db 2>&1 &
done
for db in $(seq 5 8)
...
do
	...
	(
		...
		while true
		do
			java -javaagent:dependencies/jacocoagent.jar=destfile=jacoco.${db}.exec -cp "$(pwd)/${output2}/distributions/luposdate3000/lib/*" MainKt "db${db}" $size 2
		done
	)  >> log/db$db 2>&1 &
done
wait
