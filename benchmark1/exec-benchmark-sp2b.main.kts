...
val jars = mutableListOf(
        ...
        "build-cache${Platform.getPathSeparator()}bin${Platform.getPathSeparator()}Luposdate3000_Launch_Benchmark_1-jvm.jar",
)
...
val cmd = mutableListOf("java", "-Xmx60g", "-cp", classpath, "MainKt", triplesFiles, "$minimumTime", "$numberOfTriples")
cmd.addAll(args)
ProcessBuilder(cmd)
	.directory(File("."))
