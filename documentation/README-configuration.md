...
The options "SuspendMode.Disable" or "SuspendMode.Enable" must be the same for EVERY module, because this changes method signatures in the binarys.
...
You can see, which modules are interchangeable by looking at the "modulePrefix" parameter. Modules using the same prefix are interchangeable, and are only allowed one at a time.
