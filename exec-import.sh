...
ln -s "$(pwd)/build-cache/bin/Luposdate3000_Optimizer_NoPartitions-jvm.jar" "$(pwd)/build-cache/bin-effective/Luposdate3000_Optimizer-jvm.jar"
...
ln -s "$(pwd)/build-cache/bin/Luposdate3000_Launch_Import-jvm.jar" "$(pwd)/build-cache/bin-effective/Luposdate3000_Launch_impl.jar"
...
java -Xmx60g -cp $(printf %s: $(pwd)/build-cache/bin-effective/*.jar) MainKt $@
