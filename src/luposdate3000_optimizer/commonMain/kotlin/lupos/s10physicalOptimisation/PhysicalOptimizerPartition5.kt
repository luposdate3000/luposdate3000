...
class PhysicalOptimizerPartition5(query: Query) : OptimizerBase(query, EOptimizerID.PhysicalOptimizerPartition5ID) {
    ...
    fun getNumberOfEnclosingPartitions(node: IOPBase, partitionID: Int): Int {
        var nextPartitionID = partitionID
if(partitionID!=-1){
        when (node) {
        ...
        if (childs.size > 0) {
            val tmp = getNumberOfEnclosingPartitions(childs[0], nextPartitionID)
        ...
        when (node) {
            ...
            is POPChangePartitionOrderedByIntId -> count = count * node.partitionCount
...
val count=getNumberOfEnclosingPartitions(node,node.partitionID)*node.partitionCount
...
if(newCount<query.partitionOperatorCount[node.partitionID]){
...
val count=getNumberOfEnclosingPartitions(node,node.partitionIDTo)*node.partitionCountTo
if(count>Partition.default_k){ 
val reduceFactor=count/Partition.default_k
