...
public class OperatorGraphVisitor( public val query: Query) {
    ...
    public fun visit(v0: ASTPrologue, v1: ASTClassOfInterfaceOfSelectQueryOrConstructQueryOrDescribeQueryOrAskQueryAndValuesClause): OPBaseCompound {
    ...
    public fun visit(v0: ASTPrologue, v1: ASTClassOfUpdate1AndClassOfPrologueAndUpdate): OPBaseCompound {
        val prolog = visit(v0).toMutableList()
    ...
    public fun visit(node: ASTSparqlDoc): OPBaseCompound = when (val v1 = node.variable1!!) {
        is ASTClassOfInterfaceOfSelectQueryOrConstructQueryOrDescribeQueryOrAskQueryAndValuesClause -> visit(node.variable0!!, v1)
        is ASTClassOfUpdate1AndClassOfPrologueAndUpdate -> visit(node.variable0!!, v1)
    }
...
val valuesClause= node.variable3
    ...
    private fun visit(graph: String, graphVar: Boolean, node: ASTBuiltInCall) = when (node) {
        ...
        is ASTExistsFunc -> AOPBuildInCallExists(query, visit(graph, graphVar, node.variable0!!))
        is ASTNotExistsFunc -> AOPBuildInCallNotExists(query, visit(graph, graphVar, node.variable0!!))
    ...
    private fun visit(node: ASTLimitOffsetClauses, tmp: IOPBase) = when (node) {
        is ASTClassOfLimitClauseAndOffsetClause -> {
        ...
        is ASTClassOfOffsetClauseAndLimitClause -> {
            val a = node.variable0!!
            val b = node.variable1
            if (b != null) {
    ...
    private fun visit(graph: String, graphVar: Boolean, node: ASTiriOrFunction) = if (node.variable1 != null) {
        val f = ASTFunctionCall()
        f.variable0 = node.variable0!!
        f.variable1 = node.variable1!!
    ...
    private fun visit(node: ASTRDFLiteral) = when (val v1 = node.variable1) {
        null -> {
            val buffer = ByteArrayWrapper()
    ...
    private fun visit(node: ASTBlankNode) = when (node) {
        is ASTBlankNodeLabel -> visit(node)
        is ASTBlankNodeANON -> visit(node)
    }

    private fun visit(node: ASTBlankNodeLabel): AOPConstant {
        val buffer = ByteArrayWrapper()
        DictionaryHelper.bnodeToByteArray(buffer, node.BLANK_NODE_LABEL!!)
        return AOPConstant(query, buffer)
    }

    private fun visit(unused: ASTBlankNodeANON): AOPConstant {
        val buffer = ByteArrayWrapper()
        DictionaryHelper.bnodeToByteArray(buffer, "_ASTBlankNodeANON#${counter++}")
        return AOPConstant(query, buffer)
    }

    private fun visit(node: ASTVarOrTerm) = when (node) {
        is ASTGraphTerm -> visit(node)
        is ASTVar1 -> AOPVariable(query, node.VAR1!!.drop(1))
        is ASTVar2 -> AOPVariable(query, node.VAR2!!.drop(1))
    }
    ...
    private fun visit(graph: String, graphVar: Boolean, node: ASTClassOfTriplesNodeAndPropertyList): List<LOPTriple> {
        val tmp = visit(graph, graphVar, node.variable0!!)
        if (node.variable1 != null) {
            return tmp.second + visit(graph, graphVar, tmp.first, node.variable1!!.variable0!!)
        } else {
            return tmp.second
        }
    }
    ...
    private fun visit(graph: String, graphVar: Boolean,node: ASTTriplesSameSubject): List<LOPTriple> = when (node) {
        ...
        is ASTClassOfTriplesNodeAndPropertyList -> visit(graph,graphVar,node)
    }
    ...
    private fun visit(node: ASTModify): LOPModify {
        val graph = node.variable0?.let { visit(it) } ?: ""
        val usingClause = node.variable2!!
        if (usingClause.value.size > 0) {
            TODO("using not implemented")
        }
        return when (val v1 = node.variable1!!) {
            is ASTInsertClause -> LOPModify(query, visit(graph, false, v1.variable0!!).toMutableList(), mutableListOf(), visit(graph, false, node.variable3!!))
            is ASTClassOfDeleteClauseAndInsertClause -> LOPModify(query, visit(graph, false, v1.variable1!!.variable0!!).toMutableList(), visit(graph, false, v1.variable0!!.variable0!!).toMutableList(), visit(graph, false, node.variable3!!))
        }
    }
    ...
    private fun visit(graph: String, graphVar: Boolean, node: ASTInterfaceOfSubSelectOrGroupGraphPatternSub): IOPBase = when (node) {
    ...
    private fun visit(graph: String, graphVar: Boolean, node: ASTGroupGraphPatternSub): IOPBase {
        val a: List<IOPBase> = visit(graph, graphVar, node.variable0!!)
    ...
    private fun visit(graph: String, graphVar: Boolean, node: ASTTriplesBlock): List<LOPTriple> {
        val v = node.variable1
        return if (v != null) {
            visit(graph, graphVar, node.variable0!!) + visit(graph, graphVar, v)
        } else {
            visit(graph, graphVar, node.variable0!!)
        }
    }

    private fun visit(graph: String, graphVar: Boolean, node: ASTTriplesSameSubjectPath) = when (node) {
        is ASTClassOfVarOrTermAndPropertyListPathNotEmpty -> visit(graph, graphVar, node)
        is ASTClassOfTriplesNodePathAndPropertyListPath -> visit(graph, graphVar, node)
    }
    ...
    private fun visit(node: ASTGraphNodePath): Pair<AOPBase, List<LOPTriple>> = when (node) {
        is ASTVar1 -> AOPVariable(query, node.VAR1!!.drop(1)) to listOf()
        is ASTVar2 -> AOPVariable(query, node.VAR2!!.drop(1)) to listOf()
        is ASTGraphTerm -> visit(node) to listOf()
        is ASTTriplesNodePath -> visit(node)
    }

    private fun visit(graph: String, graphVar: Boolean, node: ASTClassOfTriplesNodePathAndPropertyListPath): List<LOPTriple> {
        val tmp = visit(graph, graphVar, node.variable0!!)
        return if (node.variable1 != null) {
            tmp.second + visit(graph, graphVar, tmp.first, node.variable1!!.variable0!!)
        } else {
            tmp.second
        }
    }
    ...
    private fun visit(graph: String, graphVar: Boolean, node: ASTTriplesNodePath) = when (node) {
        is ASTBlankNodePropertyListPath -> {
            val buffer = ByteArrayWrapper()
            DictionaryHelper.bnodeToByteArray(buffer, "_ASTBlankNodePropertyList#${counter++}")
            val subject = AOPConstant(query, buffer)
            subject to visit(graph, graphVar, subject, node.variable0!!)
        }
        is ASTCollectionPath -> {
            val bufferNil = ByteArrayWrapper()
            val bufferFirst = ByteArrayWrapper()
            val bufferRest = ByteArrayWrapper()
            DictionaryHelper.iriToByteArray(bufferNil, "http://www.w3.org/1999/02/22-rdf-syntax-ns#nil")
    ...
    private fun visit(graph: String, graphVar: Boolean, node: ASTGraphPatternNotTriples, child: IOPBase): IOPBase = when (node) {
        ...
        is ASTInlineData -> visit(graph, graphVar, node, child)
    }

    private fun visit(graph: String, graphVar: Boolean, node: ASTGraphGraphPattern, child: IOPBase) = LOPJoin(query, child, when (val v0 = node.variable0!!) {
        is ASTVar1 -> visit(v0.VAR1!!.drop(1), true, node.variable1!!)
        is ASTVar2 -> visit(v0.VAR2!!.drop(1), true, node.variable1!!)
        is ASTiri -> visit(visit(v0), false, node.variable1!!)
    }, false)
...
private fun visit(graph: String, graphVar: Boolean, node: ASTClassOfVarOrTermAndPropertyListNotEmpty): List<LOPTriple> = visit(graph, graphVar, visit(node.variable0!!), node.variable1!!)
private fun visit(graph: String, graphVar: Boolean, node: ASTInlineData, child: IOPBase)=visit(node.variable0!!)
    ...
    private fun visit(node: ASTObject): Pair<AOPBase, List<LOPTriple>> = visit(node.variable0!!)
    private fun visit(graph: String, graphVar: Boolean, node: ASTQuadPattern) = visit(graph, graphVar, node.variable0!!)
    private fun visit(graph: String, graphVar: Boolean, subject: AOPBase, node: ASTListOfClassOfVerbAndObjectList): List<LOPTriple> = node.value.map { visit(graph, graphVar, subject, visit(it.variable0!!), it.variable1!!) }.flatten()
    ...
    private fun visit(graph: String, graphVar: Boolean, node: ASTClassOfQuadsNotTriplesAndpointAndTriplesTemplate) = visit(node.variable0!!) + node.variable2?.let { visit(graph, graphVar, it) }.orEmpty()
    private fun visit(graph: String, graphVar: Boolean, node: ASTListOfClassOfQuadsNotTriplesAndpointAndTriplesTemplate): List<LOPTriple> = node.value.map { visit(graph, graphVar, it) }.flatten()
    ...
    private fun visit(graph: String, graphVar: Boolean, node: ASTWhereClause) = visit(graph, graphVar, node.variable1!!)
    ...
    private fun visit(graph: String, graphVar: Boolean, subject: AOPBase, node: ASTListOfClassOfInterfaceOfVerbPathOrVerbSimpleAndObjectList): List<LOPTriple> = node.value.map { visit(graph, graphVar, subject, it) }.flatten()
    ...
    private fun visit(graph: String, graphVar: Boolean, node: ASTOptionalGraphPattern, child: IOPBase) = LOPJoin(query, child, visit(graph, graphVar, node.variable0!!), true)
    private fun visit(graph: String, graphVar: Boolean, node: ASTGroupOrUnionGraphPattern, child: IOPBase) = LOPJoin(query, child,        (            listOf(visit(graph, graphVar, node.variable0!!))                + node.variable1!!.value.map { visit(graph, graphVar, it) }).reduce { s, t -> LOPUnion(query, s, t) }, false)
    private fun visit(graph: String, graphVar: Boolean, node: ASTMinusGraphPattern, child: IOPBase) = LOPMinus(query, child, visit(graph, graphVar, node.variable0!!), mutableListOf())
    private fun visit(graph: String, graphVar: Boolean, node: ASTFilter, child: IOPBase)=LOPFilter(query,visit(graph,graphVar,node.variable0!!),child)
    private fun visit(graph: String, graphVar: Boolean, node: ASTBind, child: IOPBase)= LOPBind(query,visit(node.variable1!!),visit(graph, graphVar,node.variable0!!),child)
