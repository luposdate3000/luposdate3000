...
public class AOPAggregationSAMPLE public constructor(query: IQuery, @JvmField public val distinct: Boolean, childs: Array<AOPBase>) : AOPAggregationBase(query, EOperatorIDExt.AOPAggregationSAMPLEID, "AOPAggregationSAMPLE", ArrayAllocatorIOPBase(childs.size) { childs[it] }) {
    ...
    override fun cloneOP(): IOPBase = AOPAggregationSAMPLE(query, distinct, ArrayAllocatorIOPBase(children.size) { (children[it].cloneOP()) as AOPBase })
}
