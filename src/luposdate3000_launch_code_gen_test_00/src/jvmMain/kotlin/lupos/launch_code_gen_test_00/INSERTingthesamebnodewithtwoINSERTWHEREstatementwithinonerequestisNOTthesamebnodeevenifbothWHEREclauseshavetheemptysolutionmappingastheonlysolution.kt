...
public class INSERTingthesamebnodewithtwoINSERTWHEREstatementwithinonerequestisNOTthesamebnodeevenifbothWHEREclauseshavetheemptysolutionmappingastheonlysolution {
    ...
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - Centralized - true - Process - RPL_Fast`() {
        simulatorHelper(
    ...
    public fun getTests():Set<Pair<String,()->Unit>> {
        return setOf(
            "INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - Thread - Simple - false" to ::`INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - Thread - Simple - false`,
            "INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - Centralized - true - Process - RPL_Fast" to ::`INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - Centralized - true - Process - RPL_Fast`,
        )
    }
}
public fun main(){
