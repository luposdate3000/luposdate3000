...
public object TestCase13 {
    internal const val inputFileName=".//resources/sparql11-test-suite/aggregates/agg-numeric2.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/aggregates/agg-sum-02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'SUM with GROUP BY'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'SUM with GROUP BY' success")
        } else {
            println("Result: 'SUM with GROUP BY' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'SUM with GROUP BY' failed")
            }
        }
    }
}
