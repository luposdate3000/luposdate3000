...
public object TestCase23 {
    internal const val inputFileName=".//resources/sparql11-test-suite/aggregates/empty.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/aggregates/agg-empty-group.srx"
    @CodeGenerationAnnotation
    internal const val query = "PREFIX ex: <http://example.com/> " +
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'agg empty group' success")
        } else {
            println("Result: 'agg empty group' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'agg empty group' failed")
            }
        }
    }
}
