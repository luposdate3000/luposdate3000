...
public object TestCase219 {
    internal const val inputFileName=".//resources/sparql11-test-suite/project-expression/projexp01.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/project-expression/projexp01.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Expression is equality'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Expression is equality' success")
        } else {
            println("Result: 'Expression is equality' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Expression is equality' failed")
            }
        }
    }
}
