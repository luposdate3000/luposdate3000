...
public object TestCase161 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data-builtin-3.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/plus-1.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'plus-1' success")
        } else {
            println("Result: 'plus-1' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'plus-1' failed")
            }
        }
    }
}
