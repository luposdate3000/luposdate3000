...
public object TestCase4 {
    internal const val inputFileName=".//resources/sparql11-test-suite/aggregates/agg01.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/aggregates/agg04.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'COUNT 4' success")
        } else {
            println("Result: 'COUNT 4' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'COUNT 4' failed")
            }
        }
    }
}
