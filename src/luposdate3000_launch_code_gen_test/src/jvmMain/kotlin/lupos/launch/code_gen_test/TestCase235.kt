...
public object TestCase235 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/pp16.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/pp16.srx"
    ...
    internal operator fun invoke(){
        println("Test: '(pp16) Duplicate paths and cycles through foaf:knows*'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp16) Duplicate paths and cycles through foaf:knows*' success")
        } else {
            println("Result: '(pp16) Duplicate paths and cycles through foaf:knows*' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp16) Duplicate paths and cycles through foaf:knows*' failed")
            }
        }
    }
}
