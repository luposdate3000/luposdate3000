...
public object TestCase247 {
    internal const val inputFileName=".//resources/sparql11-test-suite/subquery/sq05.rdf"
    internal const val targetFileName=".//resources/sparql11-test-suite/subquery/sq06.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'sq06 - Subquery with graph pattern, from named applies'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'sq06 - Subquery with graph pattern, from named applies' success")
        } else {
            println("Result: 'sq06 - Subquery with graph pattern, from named applies' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'sq06 - Subquery with graph pattern, from named applies' failed")
            }
        }
    }
}
