...
public object TestCase169 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/sha512-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'SHA512()' success")
        } else {
            println("Result: 'SHA512()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'SHA512()' failed")
            }
        }
    }
}
