...
public object TestCase256 {
    ...
    internal const val query = "DROP SILENT GRAPH <http://www.example.org> " +
        ""
    internal operator fun invoke(){
        println("Test: 'DROP SILENT GRAPH iri'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'DROP SILENT GRAPH iri' success")
        } else {
            println("Result: 'DROP SILENT GRAPH iri' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'DROP SILENT GRAPH iri' failed")
            }
        }
    }
}
