...
public object TestCase76 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/rdfs04.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/rdfs04.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'RDFS inference test subClassOf'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'RDFS inference test subClassOf' success")
        } else {
            println("Result: 'RDFS inference test subClassOf' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'RDFS inference test subClassOf' failed")
            }
        }
    }
}
