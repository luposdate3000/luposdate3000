...
public object TestCase172 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/seconds-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'SECONDS()' success")
        } else {
            println("Result: 'SECONDS()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'SECONDS()' failed")
            }
        }
    }
}
