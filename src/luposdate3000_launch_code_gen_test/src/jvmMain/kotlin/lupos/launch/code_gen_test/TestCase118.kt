...
public object TestCase118 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/parent.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/parent2.srx"
    @CodeGenerationAnnotation
    internal const val query = "PREFIX : <http://example.org/test#> " +
        "SELECT ?parent " +
        "WHERE { ?parent :hasChild ?child }"
    internal operator fun invoke(){
        println("Test: 'parent query with distinguished variable'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'parent query with distinguished variable' success")
        } else {
            println("Result: 'parent query with distinguished variable' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'parent query with distinguished variable' failed")
            }
        }
    }
}
