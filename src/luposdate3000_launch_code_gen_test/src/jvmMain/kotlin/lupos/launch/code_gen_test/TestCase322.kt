...
public object TestCase322 {
    internal const val inputFileName=".//resources/myqueries/x4.ttl"
    internal const val targetFileName=".//resources/myqueries/x4.srx"
    @CodeGenerationAnnotation
    internal const val query = "PREFIX ex: <http://www.w3.org/2009/sparql/docs/tests/data-sparql11/negation#> " +
        "SELECT ?animal {  " +
        "  ?animal a ex:Animal . " +
        "    ?animal a ?type  " +
    ...
    internal operator fun invoke(){
        println("Test: './/resources/myqueries/x4.sparql-4'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: './/resources/myqueries/x4.sparql-4' success")
        } else {
            println("Result: './/resources/myqueries/x4.sparql-4' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: './/resources/myqueries/x4.sparql-4' failed")
            }
        }
    }
}
