...
public object TestCase237 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/data-diamond-tail.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/diamond-tail-2.srx"
    ...
    internal operator fun invoke(){
        println("Test: '(pp23) Diamond, with tail -- :p+'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp23) Diamond, with tail -- :p+' success")
        } else {
            println("Result: '(pp23) Diamond, with tail -- :p+' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp23) Diamond, with tail -- :p+' failed")
            }
        }
    }
}
