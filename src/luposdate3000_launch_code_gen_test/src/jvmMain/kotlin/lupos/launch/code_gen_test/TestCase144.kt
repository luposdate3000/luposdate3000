...
public object TestCase144 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/strlang03.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'STRLANG() TypeErrors'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'STRLANG() TypeErrors' success")
        } else {
            println("Result: 'STRLANG() TypeErrors' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'STRLANG() TypeErrors' failed")
            }
        }
    }
}
