...
public object TestCase220 {
    internal const val inputFileName=".//resources/sparql11-test-suite/project-expression/projexp02.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/project-expression/projexp02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Expression raise an error'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Expression raise an error' success")
        } else {
            println("Result: 'Expression raise an error' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Expression raise an error' failed")
            }
        }
    }
}
