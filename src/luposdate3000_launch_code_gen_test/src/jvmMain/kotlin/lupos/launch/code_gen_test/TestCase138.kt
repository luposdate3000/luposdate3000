...
public object TestCase138 {
    internal const val inputFileName=".//resources/sparql11-test-suite/exists/exists01.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/exists/exists05.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Nested negative exists in positive exists'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Nested negative exists in positive exists' success")
        } else {
            println("Result: 'Nested negative exists in positive exists' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Nested negative exists in positive exists' failed")
            }
        }
    }
}
