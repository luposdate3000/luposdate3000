...
public object TestCase156 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/lcase01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'LCASE()' success")
        } else {
            println("Result: 'LCASE()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'LCASE()' failed")
            }
        }
    }
}
