...
public object TestCase43 {
    internal const val inputFileName=".//resources/sparql11-test-suite/bindings/data01.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/bindings/inline01.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Inline VALUES graph pattern'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Inline VALUES graph pattern' success")
        } else {
            println("Result: 'Inline VALUES graph pattern' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Inline VALUES graph pattern' failed")
            }
        }
    }
}
