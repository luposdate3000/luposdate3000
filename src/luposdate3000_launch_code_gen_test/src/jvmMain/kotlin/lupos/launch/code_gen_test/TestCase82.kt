...
public object TestCase82 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/rdfs10.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/rdfs10.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'RDFS inference test transitivity of subPropertyOf'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'RDFS inference test transitivity of subPropertyOf' success")
        } else {
            println("Result: 'RDFS inference test transitivity of subPropertyOf' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'RDFS inference test transitivity of subPropertyOf' failed")
            }
        }
    }
}
