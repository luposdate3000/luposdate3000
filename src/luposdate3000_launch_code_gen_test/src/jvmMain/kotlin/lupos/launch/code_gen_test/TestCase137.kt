...
public object TestCase137 {
    internal const val inputFileName=".//resources/sparql11-test-suite/exists/exists01.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/exists/exists04.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Nested positive exists'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Nested positive exists' success")
        } else {
            println("Result: 'Nested positive exists' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Nested positive exists' failed")
            }
        }
    }
}
