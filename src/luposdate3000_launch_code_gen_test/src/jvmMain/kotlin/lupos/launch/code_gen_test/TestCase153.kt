...
public object TestCase153 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/substring02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'SUBSTR() (2-argument)'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'SUBSTR() (2-argument)' success")
        } else {
            println("Result: 'SUBSTR() (2-argument)' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'SUBSTR() (2-argument)' failed")
            }
        }
    }
}
