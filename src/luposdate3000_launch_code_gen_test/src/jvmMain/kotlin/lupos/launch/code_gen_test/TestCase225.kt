...
public object TestCase225 {
    internal const val inputFileName=".//resources/sparql11-test-suite/project-expression/projexp07.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/project-expression/projexp07.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Expression has variable that may be unbound'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Expression has variable that may be unbound' success")
        } else {
            println("Result: 'Expression has variable that may be unbound' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Expression has variable that may be unbound' failed")
            }
        }
    }
}
