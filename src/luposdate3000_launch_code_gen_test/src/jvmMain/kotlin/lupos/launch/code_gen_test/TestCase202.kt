...
public object TestCase202 {
    internal const val inputFileName=".//resources/sparql11-test-suite/grouping/group-data-1.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/grouping/group04.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'Group-4' success")
        } else {
            println("Result: 'Group-4' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Group-4' failed")
            }
        }
    }
}
