...
public object TestCase35 {
    internal const val inputFileName=".//resources/sparql11-test-suite/bindings/data01.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/bindings/values01.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Post-query VALUES with subj-var, 1 row'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Post-query VALUES with subj-var, 1 row' success")
        } else {
            println("Result: 'Post-query VALUES with subj-var, 1 row' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Post-query VALUES with subj-var, 1 row' failed")
            }
        }
    }
}
