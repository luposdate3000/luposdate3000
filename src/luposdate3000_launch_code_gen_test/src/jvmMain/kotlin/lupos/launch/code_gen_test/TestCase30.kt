...
public object TestCase30 {
    internal const val inputFileName=".//resources/sparql11-test-suite/bind/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/bind/bind06.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'bind06 - BIND' success")
        } else {
            println("Result: 'bind06 - BIND' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'bind06 - BIND' failed")
            }
        }
    }
}
