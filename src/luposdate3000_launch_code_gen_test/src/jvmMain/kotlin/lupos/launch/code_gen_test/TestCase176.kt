...
public object TestCase176 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/day-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'DAY()' success")
        } else {
            println("Result: 'DAY()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'DAY()' failed")
            }
        }
    }
}
