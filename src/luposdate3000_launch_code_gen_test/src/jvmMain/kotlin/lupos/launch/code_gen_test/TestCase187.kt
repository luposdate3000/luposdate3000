...
public object TestCase187 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/iri01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'IRI()/URI()' success")
        } else {
            println("Result: 'IRI()/URI()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'IRI()/URI()' failed")
            }
        }
    }
}
