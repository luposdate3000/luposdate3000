...
public object TestCase245 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/pp37.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/pp37.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: '(pp37) Nested (*)*' success")
        } else {
            println("Result: '(pp37) Nested (*)*' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp37) Nested (*)*' failed")
            }
        }
    }
}
