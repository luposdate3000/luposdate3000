...
public object TestCase44 {
    internal const val inputFileName=".//resources/sparql11-test-suite/bindings/data02.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/bindings/inline02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Post-subquery VALUES'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Post-subquery VALUES' success")
        } else {
            println("Result: 'Post-subquery VALUES' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Post-subquery VALUES' failed")
            }
        }
    }
}
