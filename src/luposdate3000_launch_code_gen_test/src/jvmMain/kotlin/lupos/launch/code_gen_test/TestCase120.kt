...
public object TestCase120 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/parent.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/parent4.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'parent query with (hasChild min 1) restriction'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'parent query with (hasChild min 1) restriction' success")
        } else {
            println("Result: 'parent query with (hasChild min 1) restriction' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'parent query with (hasChild min 1) restriction' failed")
            }
        }
    }
}
