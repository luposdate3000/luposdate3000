...
public object TestCase136 {
    internal const val inputFileName=".//resources/sparql11-test-suite/exists/exists01.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/exists/exists02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Exists with ground triple'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Exists with ground triple' success")
        } else {
            println("Result: 'Exists with ground triple' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Exists with ground triple' failed")
            }
        }
    }
}
