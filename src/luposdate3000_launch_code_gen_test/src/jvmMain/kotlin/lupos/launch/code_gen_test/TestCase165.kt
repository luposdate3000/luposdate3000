...
public object TestCase165 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/sha1-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'SHA1()' success")
        } else {
            println("Result: 'SHA1()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'SHA1()' failed")
            }
        }
    }
}
