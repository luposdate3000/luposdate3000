...
public object TestCase253 {
    ...
    internal operator fun invoke(){
        println("Test: 'sq13 - Subqueries don't inject bindings'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'sq13 - Subqueries don't inject bindings' success")
        } else {
            println("Result: 'sq13 - Subqueries don't inject bindings' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'sq13 - Subqueries don't inject bindings' failed")
            }
        }
    }
}
