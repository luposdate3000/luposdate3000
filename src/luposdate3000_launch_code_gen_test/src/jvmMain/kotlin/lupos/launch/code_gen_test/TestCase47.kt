...
public object TestCase47 {
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'constructwhere03 - CONSTRUCT WHERE' success")
        } else {
            println("Result: 'constructwhere03 - CONSTRUCT WHERE' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'constructwhere03 - CONSTRUCT WHERE' failed")
            }
        }
    }
}
