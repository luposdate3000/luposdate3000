...
public object TestCase45 {
    internal const val inputFileName=".//resources/sparql11-test-suite/construct/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/construct/constructwhere01result.ttl"
    @CodeGenerationAnnotation
    internal const val query = "PREFIX : <http://example.org/> " +
        "CONSTRUCT WHERE { ?s ?p ?o}"
    internal operator fun invoke(){
        println("Test: 'constructwhere01 - CONSTRUCT WHERE'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'constructwhere01 - CONSTRUCT WHERE' success")
        } else {
            println("Result: 'constructwhere01 - CONSTRUCT WHERE' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'constructwhere01 - CONSTRUCT WHERE' failed")
            }
        }
    }
}
