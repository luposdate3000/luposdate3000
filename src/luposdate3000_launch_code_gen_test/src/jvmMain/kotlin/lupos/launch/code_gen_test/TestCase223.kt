...
public object TestCase223 {
    internal const val inputFileName=".//resources/sparql11-test-suite/project-expression/projexp05.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/project-expression/projexp05.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Expression may return no value'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Expression may return no value' success")
        } else {
            println("Result: 'Expression may return no value' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Expression may return no value' failed")
            }
        }
    }
}
