...
public object TestCase184 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/notin02.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'NOT IN 2' success")
        } else {
            println("Result: 'NOT IN 2' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'NOT IN 2' failed")
            }
        }
    }
}
