...
public object TestCase228 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/pp03.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/pp03.srx"
    ...
    internal operator fun invoke(){
        println("Test: '(pp03) Simple path with loop'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp03) Simple path with loop' success")
        } else {
            println("Result: '(pp03) Simple path with loop' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp03) Simple path with loop' failed")
            }
        }
    }
}
