...
public object TestCase222 {
    internal const val inputFileName=".//resources/sparql11-test-suite/project-expression/projexp04.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/project-expression/projexp04.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Reuse a project expression variable in order by'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Reuse a project expression variable in order by' success")
        } else {
            println("Result: 'Reuse a project expression variable in order by' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Reuse a project expression variable in order by' failed")
            }
        }
    }
}
