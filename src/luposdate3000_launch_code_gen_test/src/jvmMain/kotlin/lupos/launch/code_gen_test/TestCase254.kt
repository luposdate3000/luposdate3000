...
public object TestCase254 {
    internal const val inputFileName=".//resources/sparql11-test-suite/subquery/sq14.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/subquery/sq14-out.ttl"
    ...
    internal operator fun invoke(){
        println("Test: 'sq14 - limit by resource'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'sq14 - limit by resource' success")
        } else {
            println("Result: 'sq14 - limit by resource' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'sq14 - limit by resource' failed")
            }
        }
    }
}
