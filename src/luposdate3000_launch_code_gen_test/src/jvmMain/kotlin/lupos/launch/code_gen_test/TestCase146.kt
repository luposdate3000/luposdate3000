...
public object TestCase146 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/abs01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'ABS()' success")
        } else {
            println("Result: 'ABS()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'ABS()' failed")
            }
        }
    }
}
