...
public object TestCase117 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/lang.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/lang.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Literal with language tag test'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Literal with language tag test' success")
        } else {
            println("Result: 'Literal with language tag test' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Literal with language tag test' failed")
            }
        }
    }
}
