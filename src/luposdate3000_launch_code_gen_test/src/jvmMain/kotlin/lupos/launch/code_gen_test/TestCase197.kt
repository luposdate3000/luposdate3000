...
public object TestCase197 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data3.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/replace03.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'REPLACE() with captured substring'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'REPLACE() with captured substring' success")
        } else {
            println("Result: 'REPLACE() with captured substring' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'REPLACE() with captured substring' failed")
            }
        }
    }
}
