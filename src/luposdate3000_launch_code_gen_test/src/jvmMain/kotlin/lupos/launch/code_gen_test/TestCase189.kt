...
public object TestCase189 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data2.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/if02.srx"
    @CodeGenerationAnnotation
    internal const val query = "SELECT (IF(1/0, false, true) AS ?error) WHERE {} " +
        ""
    internal operator fun invoke(){
        println("Test: 'IF() error propogation'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'IF() error propogation' success")
        } else {
            println("Result: 'IF() error propogation' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'IF() error propogation' failed")
            }
        }
    }
}
