...
public object TestCase80 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/rdfs08.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/rdfs08.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'RDFS inference test rdf:XMLLiteral'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'RDFS inference test rdf:XMLLiteral' success")
        } else {
            println("Result: 'RDFS inference test rdf:XMLLiteral' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'RDFS inference test rdf:XMLLiteral' failed")
            }
        }
    }
}
