...
public object TestCase190 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data-coalesce.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/coalesce01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'COALESCE()' success")
        } else {
            println("Result: 'COALESCE()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'COALESCE()' failed")
            }
        }
    }
}
