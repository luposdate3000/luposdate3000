...
public object TestCase37 {
    internal const val inputFileName=".//resources/sparql11-test-suite/bindings/data03.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/bindings/values03.srx"
    @CodeGenerationAnnotation
    internal const val query = "# bindings with two variables and one set of values " +
    ...
    internal operator fun invoke(){
        println("Test: 'Post-query VALUES with 2 obj-vars, 1 row'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Post-query VALUES with 2 obj-vars, 1 row' success")
        } else {
            println("Result: 'Post-query VALUES with 2 obj-vars, 1 row' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Post-query VALUES with 2 obj-vars, 1 row' failed")
            }
        }
    }
}
