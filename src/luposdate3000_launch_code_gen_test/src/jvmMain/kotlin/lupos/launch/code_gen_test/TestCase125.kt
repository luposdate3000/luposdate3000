...
public object TestCase125 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/parent.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/parent9.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'subclass query with (hasChild some Thing) restriction'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'subclass query with (hasChild some Thing) restriction' success")
        } else {
            println("Result: 'subclass query with (hasChild some Thing) restriction' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'subclass query with (hasChild some Thing) restriction' failed")
            }
        }
    }
}
