...
public object TestCase71 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/rdf03.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/rdf03.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'RDF test for blank node cardinalities'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'RDF test for blank node cardinalities' success")
        } else {
            println("Result: 'RDF test for blank node cardinalities' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'RDF test for blank node cardinalities' failed")
            }
        }
    }
}
