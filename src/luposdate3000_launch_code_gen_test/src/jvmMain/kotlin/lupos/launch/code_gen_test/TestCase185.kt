...
public object TestCase185 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/now01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'NOW()' success")
        } else {
            println("Result: 'NOW()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'NOW()' failed")
            }
        }
    }
}
