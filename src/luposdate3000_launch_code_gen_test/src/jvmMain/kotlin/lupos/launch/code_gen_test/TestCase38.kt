...
public object TestCase38 {
    internal const val inputFileName=".//resources/sparql11-test-suite/bindings/data04.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/bindings/values04.srx"
    @CodeGenerationAnnotation
    internal const val query = "# bindings with one element UNDEF " +
    ...
    internal operator fun invoke(){
        println("Test: 'Post-query VALUES with 2 obj-vars, 1 row with UNDEF'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Post-query VALUES with 2 obj-vars, 1 row with UNDEF' success")
        } else {
            println("Result: 'Post-query VALUES with 2 obj-vars, 1 row with UNDEF' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Post-query VALUES with 2 obj-vars, 1 row with UNDEF' failed")
            }
        }
    }
}
