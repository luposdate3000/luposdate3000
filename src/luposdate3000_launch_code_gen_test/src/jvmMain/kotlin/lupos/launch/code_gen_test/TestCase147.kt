...
public object TestCase147 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/ceil01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'CEIL()' success")
        } else {
            println("Result: 'CEIL()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'CEIL()' failed")
            }
        }
    }
}
