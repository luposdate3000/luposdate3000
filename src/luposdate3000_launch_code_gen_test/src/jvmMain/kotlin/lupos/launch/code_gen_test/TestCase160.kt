...
public object TestCase160 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/ends01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'STRENDS()' success")
        } else {
            println("Result: 'STRENDS()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'STRENDS()' failed")
            }
        }
    }
}
