...
public object TestCase70 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/rdf02.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/rdf02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'RDF inference test'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'RDF inference test' success")
        } else {
            println("Result: 'RDF inference test' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'RDF inference test' failed")
            }
        }
    }
}
