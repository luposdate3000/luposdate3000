...
public object TestCase49 {
    internal const val inputFileName=".//resources/sparql11-test-suite/csv-tsv-res/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/csv-tsv-res/csvtsv01.tsv"
    @CodeGenerationAnnotation
    internal const val query = "PREFIX : <http://example.org/> " +
        "SELECT * WHERE { ?s ?p ?o} ORDER BY ?s ?p ?o"
    internal operator fun invoke(){
        println("Test: 'tsv01 - TSV Result Format'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'tsv01 - TSV Result Format' success")
        } else {
            println("Result: 'tsv01 - TSV Result Format' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'tsv01 - TSV Result Format' failed")
            }
        }
    }
}
