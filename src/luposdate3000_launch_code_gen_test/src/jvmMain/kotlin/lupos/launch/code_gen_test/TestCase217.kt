...
public object TestCase217 {
    internal const val inputFileName=".//resources/sparql11-test-suite/negation/full-minuend.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/negation/full-minuend.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Subtraction with MINUS from a fully bound minuend'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Subtraction with MINUS from a fully bound minuend' success")
        } else {
            println("Result: 'Subtraction with MINUS from a fully bound minuend' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Subtraction with MINUS from a fully bound minuend' failed")
            }
        }
    }
}
