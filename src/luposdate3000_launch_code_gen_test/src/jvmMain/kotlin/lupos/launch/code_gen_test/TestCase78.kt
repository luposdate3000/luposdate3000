...
public object TestCase78 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/rdfs06.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/rdfs06.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'RDFS inference test domain'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'RDFS inference test domain' success")
        } else {
            println("Result: 'RDFS inference test domain' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'RDFS inference test domain' failed")
            }
        }
    }
}
