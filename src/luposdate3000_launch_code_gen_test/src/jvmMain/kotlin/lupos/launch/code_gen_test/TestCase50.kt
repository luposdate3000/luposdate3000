...
public object TestCase50 {
    internal const val inputFileName=".//resources/sparql11-test-suite/csv-tsv-res/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/csv-tsv-res/csvtsv02.csv"
    ...
    internal operator fun invoke(){
        println("Test: 'cvs02 - CSV Result Format'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'cvs02 - CSV Result Format' success")
        } else {
            println("Result: 'cvs02 - CSV Result Format' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'cvs02 - CSV Result Format' failed")
            }
        }
    }
}
