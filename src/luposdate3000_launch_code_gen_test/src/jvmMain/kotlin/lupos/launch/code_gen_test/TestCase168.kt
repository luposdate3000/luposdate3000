...
public object TestCase168 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/hash-unicode.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/sha256-02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'SHA256() on Unicode data'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'SHA256() on Unicode data' success")
        } else {
            println("Result: 'SHA256() on Unicode data' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'SHA256() on Unicode data' failed")
            }
        }
    }
}
