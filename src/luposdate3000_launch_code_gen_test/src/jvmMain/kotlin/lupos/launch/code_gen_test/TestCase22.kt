...
public object TestCase22 {
    internal const val inputFileName=".//resources/sparql11-test-suite/aggregates/agg-err-02.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/aggregates/agg-err-02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Protect from error in AVG'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Protect from error in AVG' success")
        } else {
            println("Result: 'Protect from error in AVG' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Protect from error in AVG' failed")
            }
        }
    }
}
