...
public object TestCase51 {
    internal const val inputFileName=".//resources/sparql11-test-suite/csv-tsv-res/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/csv-tsv-res/csvtsv02.tsv"
    ...
    internal operator fun invoke(){
        println("Test: 'tvs02 - TSV Result Format'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'tvs02 - TSV Result Format' success")
        } else {
            println("Result: 'tvs02 - TSV Result Format' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'tvs02 - TSV Result Format' failed")
            }
        }
    }
}
