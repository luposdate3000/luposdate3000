...
public object TestCase199 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data-empty.nt"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/struuid01.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'STRUUID() pattern match'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'STRUUID() pattern match' success")
        } else {
            println("Result: 'STRUUID() pattern match' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'STRUUID() pattern match' failed")
            }
        }
    }
}
