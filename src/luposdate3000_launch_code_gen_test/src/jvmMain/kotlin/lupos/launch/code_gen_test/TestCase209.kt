...
public object TestCase209 {
    internal const val inputFileName=".//resources/sparql11-test-suite/negation/subsetByExcl.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/negation/subsetByExcl02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Subsets by exclusion (MINUS)'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Subsets by exclusion (MINUS)' success")
        } else {
            println("Result: 'Subsets by exclusion (MINUS)' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Subsets by exclusion (MINUS)' failed")
            }
        }
    }
}
