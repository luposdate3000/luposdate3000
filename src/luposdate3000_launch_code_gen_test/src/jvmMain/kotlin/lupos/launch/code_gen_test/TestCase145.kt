...
public object TestCase145 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/isnumeric01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'isNumeric()' success")
        } else {
            println("Result: 'isNumeric()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'isNumeric()' failed")
            }
        }
    }
}
