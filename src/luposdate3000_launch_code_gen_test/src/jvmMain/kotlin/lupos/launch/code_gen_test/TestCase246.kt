...
public object TestCase246 {
    internal const val inputFileName=".//resources/sparql11-test-suite/service/data07.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/service/service07.srx"
    @CodeGenerationAnnotation
    internal const val query = "# invalid URI for a SERVICE with SILENT " +
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'SERVICE test 7' success")
        } else {
            println("Result: 'SERVICE test 7' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'SERVICE test 7' failed")
            }
        }
    }
}
