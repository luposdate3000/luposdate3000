...
public object TestCase191 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data2.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/strbefore01a.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'STRBEFORE()' success")
        } else {
            println("Result: 'STRBEFORE()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'STRBEFORE()' failed")
            }
        }
    }
}
