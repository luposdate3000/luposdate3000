...
public object TestCase208 {
    internal const val inputFileName=".//resources/sparql11-test-suite/negation/subsetByExcl.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/negation/subsetByExcl01.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Subsets by exclusion (NOT EXISTS)'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Subsets by exclusion (NOT EXISTS)' success")
        } else {
            println("Result: 'Subsets by exclusion (NOT EXISTS)' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Subsets by exclusion (NOT EXISTS)' failed")
            }
        }
    }
}
