...
public object TestCase232 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/pp11.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/pp11.srx"
    ...
    internal operator fun invoke(){
        println("Test: '(pp11) Simple path and two paths to same target node'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp11) Simple path and two paths to same target node' success")
        } else {
            println("Result: '(pp11) Simple path and two paths to same target node' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp11) Simple path and two paths to same target node' failed")
            }
        }
    }
}
