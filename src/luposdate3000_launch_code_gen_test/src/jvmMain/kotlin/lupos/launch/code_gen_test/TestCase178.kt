...
public object TestCase178 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/tz-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'TZ()' success")
        } else {
            println("Result: 'TZ()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'TZ()' failed")
            }
        }
    }
}
