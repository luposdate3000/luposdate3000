...
public object TestCase196 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data3.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/replace02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'REPLACE() with overlapping pattern'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'REPLACE() with overlapping pattern' success")
        } else {
            println("Result: 'REPLACE() with overlapping pattern' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'REPLACE() with overlapping pattern' failed")
            }
        }
    }
}
