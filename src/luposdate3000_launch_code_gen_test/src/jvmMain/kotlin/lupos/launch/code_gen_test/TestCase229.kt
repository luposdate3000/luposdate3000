...
public object TestCase229 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/pp08.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/pp08.srx"
    ...
    internal operator fun invoke(){
        println("Test: '(pp08) Reverse path'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp08) Reverse path' success")
        } else {
            println("Result: '(pp08) Reverse path' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp08) Reverse path' failed")
            }
        }
    }
}
