...
public object TestCase19 {
    internal const val inputFileName=".//resources/sparql11-test-suite/aggregates/agg-numeric.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/aggregates/agg-max-02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'MAX with GROUP BY'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'MAX with GROUP BY' success")
        } else {
            println("Result: 'MAX with GROUP BY' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'MAX with GROUP BY' failed")
            }
        }
    }
}
