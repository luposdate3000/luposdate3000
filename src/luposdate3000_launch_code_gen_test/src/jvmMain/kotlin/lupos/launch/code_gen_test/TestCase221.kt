...
public object TestCase221 {
    internal const val inputFileName=".//resources/sparql11-test-suite/project-expression/projexp03.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/project-expression/projexp03.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Reuse a project expression variable in select'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Reuse a project expression variable in select' success")
        } else {
            println("Result: 'Reuse a project expression variable in select' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Reuse a project expression variable in select' failed")
            }
        }
    }
}
