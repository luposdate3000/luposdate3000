...
public object TestCase171 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/minutes-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'MINUTES()' success")
        } else {
            println("Result: 'MINUTES()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'MINUTES()' failed")
            }
        }
    }
}
