...
public object TestCase150 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/concat01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'CONCAT()' success")
        } else {
            println("Result: 'CONCAT()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'CONCAT()' failed")
            }
        }
    }
}
