...
public object TestCase159 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/starts01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'STRSTARTS()' success")
        } else {
            println("Result: 'STRSTARTS()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'STRSTARTS()' failed")
            }
        }
    }
}
