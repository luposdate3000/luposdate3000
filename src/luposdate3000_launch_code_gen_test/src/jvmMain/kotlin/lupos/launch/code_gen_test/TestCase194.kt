...
public object TestCase194 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data4.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/strafter02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'STRAFTER() datatyping'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'STRAFTER() datatyping' success")
        } else {
            println("Result: 'STRAFTER() datatyping' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'STRAFTER() datatyping' failed")
            }
        }
    }
}
