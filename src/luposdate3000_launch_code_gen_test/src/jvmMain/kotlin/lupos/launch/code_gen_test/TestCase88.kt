...
public object TestCase88 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/owlds02.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/owlds02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'bnodes are not existentials with answer'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'bnodes are not existentials with answer' success")
        } else {
            println("Result: 'bnodes are not existentials with answer' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'bnodes are not existentials with answer' failed")
            }
        }
    }
}
