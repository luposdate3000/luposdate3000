...
public object TestCase135 {
    internal const val inputFileName=".//resources/sparql11-test-suite/exists/exists01.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/exists/exists01.srx"
    @CodeGenerationAnnotation
    internal const val query = "prefix ex: <http://www.example.org/> " +
        "select * where { " +
        "?s ?p ?o " +
    ...
    internal operator fun invoke(){
        println("Test: 'Exists with one constant'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Exists with one constant' success")
        } else {
            println("Result: 'Exists with one constant' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Exists with one constant' failed")
            }
        }
    }
}
