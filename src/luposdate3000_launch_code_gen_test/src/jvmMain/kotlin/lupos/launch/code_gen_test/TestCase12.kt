...
public object TestCase12 {
    internal const val inputFileName=".//resources/sparql11-test-suite/aggregates/agg-numeric.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/aggregates/agg-sum-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'SUM' success")
        } else {
            println("Result: 'SUM' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'SUM' failed")
            }
        }
    }
}
