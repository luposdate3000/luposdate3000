...
public object TestCase453 {
    internal const val inputFileName=".//resources/btc/./020.sparql.n3"
    internal const val targetFileName=".//resources/btc/./020.sparql.srx"
    @CodeGenerationAnnotation
    internal const val query = "#Parallelizing Join Computations of SPARQL Queries for Large Semantic Web Databases --- BTC 2009 " +
        "select * where { " +
    ...
    internal operator fun invoke(){
        println("Test: './/resources/btc/./020.sparql-80'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: './/resources/btc/./020.sparql-80' success")
        } else {
            println("Result: './/resources/btc/./020.sparql-80' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: './/resources/btc/./020.sparql-80' failed")
            }
        }
    }
}
