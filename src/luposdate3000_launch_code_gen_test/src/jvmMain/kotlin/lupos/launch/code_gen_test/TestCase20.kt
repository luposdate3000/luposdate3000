...
public object TestCase20 {
    internal const val inputFileName=".//resources/sparql11-test-suite/aggregates/agg-numeric.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/aggregates/agg-sample-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'SAMPLE' success")
        } else {
            println("Result: 'SAMPLE' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'SAMPLE' failed")
            }
        }
    }
}
