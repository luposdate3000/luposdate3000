...
public object TestCase84 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/rdfs12.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/rdfs12.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'RDFS inference test containers'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'RDFS inference test containers' success")
        } else {
            println("Result: 'RDFS inference test containers' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'RDFS inference test containers' failed")
            }
        }
    }
}
