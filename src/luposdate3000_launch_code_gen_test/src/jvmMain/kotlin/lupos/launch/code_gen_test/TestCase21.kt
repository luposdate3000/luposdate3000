...
public object TestCase21 {
    internal const val inputFileName=".//resources/sparql11-test-suite/aggregates/agg-err-01.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/aggregates/agg-err-01.srx"
    @CodeGenerationAnnotation
    internal const val query = "PREFIX : <http://example.com/data/#> " +
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'Error in AVG' success")
        } else {
            println("Result: 'Error in AVG' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Error in AVG' failed")
            }
        }
    }
}
