...
public object TestCase252 {
    internal const val inputFileName=".//resources/sparql11-test-suite/subquery/sq12.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/subquery/sq12_out.ttl"
    @CodeGenerationAnnotation
    internal const val query = "PREFIX foaf: <http://xmlns.com/foaf/0.1/> " +
    ...
    internal operator fun invoke(){
        println("Test: 'sq12 - Subquery in CONSTRUCT with built-ins'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'sq12 - Subquery in CONSTRUCT with built-ins' success")
        } else {
            println("Result: 'sq12 - Subquery in CONSTRUCT with built-ins' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'sq12 - Subquery in CONSTRUCT with built-ins' failed")
            }
        }
    }
}
