...
public object TestCase83 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/rdfs11.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/rdfs11.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'RDFS inference test subProperty and instances'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'RDFS inference test subProperty and instances' success")
        } else {
            println("Result: 'RDFS inference test subProperty and instances' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'RDFS inference test subProperty and instances' failed")
            }
        }
    }
}
