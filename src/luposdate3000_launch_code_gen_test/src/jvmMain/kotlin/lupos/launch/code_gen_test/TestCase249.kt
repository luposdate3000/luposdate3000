...
public object TestCase249 {
    internal const val inputFileName=".//resources/sparql11-test-suite/subquery/sq09.rdf"
    internal const val targetFileName=".//resources/sparql11-test-suite/subquery/sq09.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'sq09 - Nested Subqueries'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'sq09 - Nested Subqueries' success")
        } else {
            println("Result: 'sq09 - Nested Subqueries' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'sq09 - Nested Subqueries' failed")
            }
        }
    }
}
