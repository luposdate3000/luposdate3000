...
public object TestCase195 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data3.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/replace01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'REPLACE()' success")
        } else {
            println("Result: 'REPLACE()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'REPLACE()' failed")
            }
        }
    }
}
