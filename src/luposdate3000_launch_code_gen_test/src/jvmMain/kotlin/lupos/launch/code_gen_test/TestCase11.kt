...
public object TestCase11 {
    internal const val inputFileName=".//resources/sparql11-test-suite/aggregates/agg-groupconcat-1.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/aggregates/agg-groupconcat-3.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'GROUP_CONCAT with SEPARATOR'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'GROUP_CONCAT with SEPARATOR' success")
        } else {
            println("Result: 'GROUP_CONCAT with SEPARATOR' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'GROUP_CONCAT with SEPARATOR' failed")
            }
        }
    }
}
