...
public object TestCase180 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/bnode02.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'BNODE()' success")
        } else {
            println("Result: 'BNODE()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'BNODE()' failed")
            }
        }
    }
}
