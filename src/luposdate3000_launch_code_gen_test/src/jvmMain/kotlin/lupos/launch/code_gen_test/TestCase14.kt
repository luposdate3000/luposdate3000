...
public object TestCase14 {
    internal const val inputFileName=".//resources/sparql11-test-suite/aggregates/agg-numeric.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/aggregates/agg-avg-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'AVG' success")
        } else {
            println("Result: 'AVG' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'AVG' failed")
            }
        }
    }
}
