...
public object TestCase154 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/length01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'STRLEN()' success")
        } else {
            println("Result: 'STRLEN()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'STRLEN()' failed")
            }
        }
    }
}
