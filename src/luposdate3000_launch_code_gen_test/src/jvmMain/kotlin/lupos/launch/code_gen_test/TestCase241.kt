...
public object TestCase241 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/path-p1.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/path-p2.srx"
    ...
    internal operator fun invoke(){
        println("Test: '(pp31) Operator precedence 2'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp31) Operator precedence 2' success")
        } else {
            println("Result: '(pp31) Operator precedence 2' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp31) Operator precedence 2' failed")
            }
        }
    }
}
