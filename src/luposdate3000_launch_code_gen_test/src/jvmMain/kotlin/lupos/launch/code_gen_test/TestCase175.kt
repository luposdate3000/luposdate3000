...
public object TestCase175 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/year-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'YEAR()' success")
        } else {
            println("Result: 'YEAR()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'YEAR()' failed")
            }
        }
    }
}
