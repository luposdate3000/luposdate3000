...
public object TestCase193 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data2.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/strafter01a.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'STRAFTER()' success")
        } else {
            println("Result: 'STRAFTER()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'STRAFTER()' failed")
            }
        }
    }
}
