...
public object TestCase204 {
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'jsonres01 - JSON Result Format' success")
        } else {
            println("Result: 'jsonres01 - JSON Result Format' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'jsonres01 - JSON Result Format' failed")
            }
        }
    }
}
