...
public object TestCase31 {
    internal const val inputFileName=".//resources/sparql11-test-suite/bind/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/bind/bind07.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'bind07 - BIND' success")
        } else {
            println("Result: 'bind07 - BIND' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'bind07 - BIND' failed")
            }
        }
    }
}
