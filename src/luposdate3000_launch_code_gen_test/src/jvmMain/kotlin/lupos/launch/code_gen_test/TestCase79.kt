...
public object TestCase79 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/rdfs07.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/rdfs07.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'RDFS inference test range'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'RDFS inference test range' success")
        } else {
            println("Result: 'RDFS inference test range' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'RDFS inference test range' failed")
            }
        }
    }
}
