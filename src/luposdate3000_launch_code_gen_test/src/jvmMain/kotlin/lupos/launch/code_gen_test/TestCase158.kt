...
public object TestCase158 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/contains01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'CONTAINS()' success")
        } else {
            println("Result: 'CONTAINS()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'CONTAINS()' failed")
            }
        }
    }
}
