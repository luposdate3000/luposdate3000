...
public object TestCase214 {
    internal const val inputFileName=".//resources/sparql11-test-suite/negation/set-data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/negation/subset-03.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Calculate proper subset'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Calculate proper subset' success")
        } else {
            println("Result: 'Calculate proper subset' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Calculate proper subset' failed")
            }
        }
    }
}
