...
public object TestCase198 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data-empty.nt"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/uuid01.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'UUID() pattern match'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'UUID() pattern match' success")
        } else {
            println("Result: 'UUID() pattern match' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'UUID() pattern match' failed")
            }
        }
    }
}
