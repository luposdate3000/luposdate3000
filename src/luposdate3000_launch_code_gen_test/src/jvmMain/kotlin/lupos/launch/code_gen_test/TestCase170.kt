...
public object TestCase170 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/hash-unicode.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/sha512-02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'SHA512() on Unicode data'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'SHA512() on Unicode data' success")
        } else {
            println("Result: 'SHA512() on Unicode data' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'SHA512() on Unicode data' failed")
            }
        }
    }
}
