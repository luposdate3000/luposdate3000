...
public object TestCase87 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/owlds01.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/owlds01.srx"
    @CodeGenerationAnnotation
    internal const val query = "PREFIX   ex:  <http://example.org/x/> " +
    ...
    internal operator fun invoke(){
        println("Test: 'bnodes are not existentials'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'bnodes are not existentials' success")
        } else {
            println("Result: 'bnodes are not existentials' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'bnodes are not existentials' failed")
            }
        }
    }
}
