...
public object TestCase74 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/rdfs01.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/rdfs02.srx"
    @CodeGenerationAnnotation
    internal const val query = "PREFIX ex: <http://example.org/ns#> " +
        "SELECT ?x " +
        "WHERE { " +
        "  ?x ex:b2 ex:c . " +
        "} " +
    ...
    internal operator fun invoke(){
        println("Test: 'RDFS inference test rdfs:subPropertyOf'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'RDFS inference test rdfs:subPropertyOf' success")
        } else {
            println("Result: 'RDFS inference test rdfs:subPropertyOf' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'RDFS inference test rdfs:subPropertyOf' failed")
            }
        }
    }
}
