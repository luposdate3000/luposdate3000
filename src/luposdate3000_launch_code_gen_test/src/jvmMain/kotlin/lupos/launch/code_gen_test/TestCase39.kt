...
public object TestCase39 {
    internal const val inputFileName=".//resources/sparql11-test-suite/bindings/data05.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/bindings/values05.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Post-query VALUES with 2 obj-vars, 2 rows with UNDEF'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Post-query VALUES with 2 obj-vars, 2 rows with UNDEF' success")
        } else {
            println("Result: 'Post-query VALUES with 2 obj-vars, 2 rows with UNDEF' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Post-query VALUES with 2 obj-vars, 2 rows with UNDEF' failed")
            }
        }
    }
}
