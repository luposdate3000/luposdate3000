...
public object TestCase452 {
    internal const val inputFileName=".//resources/btc/./002.sparql.n3"
    internal const val targetFileName=".//resources/btc/./002.sparql.srx"
    @CodeGenerationAnnotation
    internal const val query = "#Scalable Join Processing on Very Large RDF Graphs --- BTC 2008 " +
        "select ?b ?p ?bn where { " +
        " ?a ?c \"Tim Berners-Lee\" . " +
    ...
    internal operator fun invoke(){
        println("Test: './/resources/btc/./002.sparql-46'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: './/resources/btc/./002.sparql-46' success")
        } else {
            println("Result: './/resources/btc/./002.sparql-46' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: './/resources/btc/./002.sparql-46' failed")
            }
        }
    }
}
