...
public object TestCase240 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/path-p1.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/path-p1.srx"
    ...
    internal operator fun invoke(){
        println("Test: '(pp30) Operator precedence 1'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp30) Operator precedence 1' success")
        } else {
            println("Result: '(pp30) Operator precedence 1' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp30) Operator precedence 1' failed")
            }
        }
    }
}
