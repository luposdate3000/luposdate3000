...
public object TestCase15 {
    internal const val inputFileName=".//resources/sparql11-test-suite/aggregates/agg-numeric2.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/aggregates/agg-avg-02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'AVG with GROUP BY'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'AVG with GROUP BY' success")
        } else {
            println("Result: 'AVG with GROUP BY' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'AVG with GROUP BY' failed")
            }
        }
    }
}
