...
public object TestCase200 {
    internal const val inputFileName=".//resources/sparql11-test-suite/grouping/group-data-1.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/grouping/group01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'Group-1' success")
        } else {
            println("Result: 'Group-1' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Group-1' failed")
            }
        }
    }
}
