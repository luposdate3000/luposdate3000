...
public object TestCase164 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/md5-02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'MD5() over Unicode data'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'MD5() over Unicode data' success")
        } else {
            println("Result: 'MD5() over Unicode data' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'MD5() over Unicode data' failed")
            }
        }
    }
}
