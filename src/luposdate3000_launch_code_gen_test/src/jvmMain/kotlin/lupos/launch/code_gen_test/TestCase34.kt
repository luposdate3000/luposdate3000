...
public object TestCase34 {
    internal const val inputFileName=".//resources/sparql11-test-suite/bind/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/bind/bind11.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'bind11 - BIND scoping - Variable in filter in scope'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'bind11 - BIND scoping - Variable in filter in scope' success")
        } else {
            println("Result: 'bind11 - BIND scoping - Variable in filter in scope' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'bind11 - BIND scoping - Variable in filter in scope' failed")
            }
        }
    }
}
