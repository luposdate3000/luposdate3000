...
public object TestCase72 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/rdf04.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/rdf04.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'simple triple pattern match'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'simple triple pattern match' success")
        } else {
            println("Result: 'simple triple pattern match' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'simple triple pattern match' failed")
            }
        }
    }
}
