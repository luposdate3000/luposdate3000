...
public object TestCase248 {
    internal const val inputFileName=".//resources/sparql11-test-suite/subquery/sq08.rdf"
    internal const val targetFileName=".//resources/sparql11-test-suite/subquery/sq08.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'sq08 - Subquery with aggregate'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'sq08 - Subquery with aggregate' success")
        } else {
            println("Result: 'sq08 - Subquery with aggregate' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'sq08 - Subquery with aggregate' failed")
            }
        }
    }
}
