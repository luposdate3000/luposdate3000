...
public object TestCase242 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/path-p3.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/path-p3.srx"
    ...
    internal operator fun invoke(){
        println("Test: '(pp32) Operator precedence 3'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp32) Operator precedence 3' success")
        } else {
            println("Result: '(pp32) Operator precedence 3' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp32) Operator precedence 3' failed")
            }
        }
    }
}
