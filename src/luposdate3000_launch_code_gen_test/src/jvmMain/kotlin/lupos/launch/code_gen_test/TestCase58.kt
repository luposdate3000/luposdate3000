...
public object TestCase58 {
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'DELETE INSERT 1c' success")
        } else {
            println("Result: 'DELETE INSERT 1c' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'DELETE INSERT 1c' failed")
            }
        }
    }
}
