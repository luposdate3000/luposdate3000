...
public object TestCase188 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data2.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/if01.srx"
    @CodeGenerationAnnotation
    internal const val query = "BASE <http://example.org/> " +
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'IF()' success")
        } else {
            println("Result: 'IF()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'IF()' failed")
            }
        }
    }
}
