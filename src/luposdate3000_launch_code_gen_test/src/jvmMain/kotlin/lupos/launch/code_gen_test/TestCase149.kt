...
public object TestCase149 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/round01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'ROUND()' success")
        } else {
            println("Result: 'ROUND()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'ROUND()' failed")
            }
        }
    }
}
