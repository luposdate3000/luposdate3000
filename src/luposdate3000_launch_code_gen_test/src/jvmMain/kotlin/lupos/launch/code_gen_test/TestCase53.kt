...
public object TestCase53 {
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'tsv03 - TSV Result Format' success")
        } else {
            println("Result: 'tsv03 - TSV Result Format' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'tsv03 - TSV Result Format' failed")
            }
        }
    }
}
