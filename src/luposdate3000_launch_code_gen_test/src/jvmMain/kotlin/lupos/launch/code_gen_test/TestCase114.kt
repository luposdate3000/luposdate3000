...
public object TestCase114 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/data-11.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/sparqldl-11.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'sparqldl-11.rq: domain test'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'sparqldl-11.rq: domain test' success")
        } else {
            println("Result: 'sparqldl-11.rq: domain test' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'sparqldl-11.rq: domain test' failed")
            }
        }
    }
}
