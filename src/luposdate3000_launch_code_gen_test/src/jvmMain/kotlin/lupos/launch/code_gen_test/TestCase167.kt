...
public object TestCase167 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/sha256-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'SHA256()' success")
        } else {
            println("Result: 'SHA256()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'SHA256()' failed")
            }
        }
    }
}
