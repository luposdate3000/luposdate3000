...
public object TestCase181 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/in01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'IN 1' success")
        } else {
            println("Result: 'IN 1' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'IN 1' failed")
            }
        }
    }
}
