...
public object TestCase140 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/strdt02.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'STRDT(STR())' success")
        } else {
            println("Result: 'STRDT(STR())' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'STRDT(STR())' failed")
            }
        }
    }
}
