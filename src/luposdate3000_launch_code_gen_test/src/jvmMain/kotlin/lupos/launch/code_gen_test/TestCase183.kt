...
public object TestCase183 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/notin01.srx"
    @CodeGenerationAnnotation
    internal const val query = "ASK { " +
        " FILTER(2 NOT IN ()) " +
        "} " +
        ""
    internal operator fun invoke(){
        println("Test: 'NOT IN 1'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'NOT IN 1' success")
        } else {
            println("Result: 'NOT IN 1' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'NOT IN 1' failed")
            }
        }
    }
}
