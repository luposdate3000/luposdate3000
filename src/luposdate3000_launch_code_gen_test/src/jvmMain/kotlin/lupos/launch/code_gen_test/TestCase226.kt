...
public object TestCase226 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/pp01.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/pp01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: '(pp01) Simple path' success")
        } else {
            println("Result: '(pp01) Simple path' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp01) Simple path' failed")
            }
        }
    }
}
