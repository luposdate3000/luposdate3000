...
public object TestCase17 {
    internal const val inputFileName=".//resources/sparql11-test-suite/aggregates/agg-numeric.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/aggregates/agg-min-02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'MIN with GROUP BY'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'MIN with GROUP BY' success")
        } else {
            println("Result: 'MIN with GROUP BY' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'MIN with GROUP BY' failed")
            }
        }
    }
}
