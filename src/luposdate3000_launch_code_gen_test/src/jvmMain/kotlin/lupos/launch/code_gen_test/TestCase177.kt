...
public object TestCase177 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/timezone-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'TIMEZONE()' success")
        } else {
            println("Result: 'TIMEZONE()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'TIMEZONE()' failed")
            }
        }
    }
}
