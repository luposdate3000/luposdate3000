...
public object TestCase243 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/path-p3.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/path-p4.srx"
    ...
    internal operator fun invoke(){
        println("Test: '(pp33) Operator precedence 4'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp33) Operator precedence 4' success")
        } else {
            println("Result: '(pp33) Operator precedence 4' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp33) Operator precedence 4' failed")
            }
        }
    }
}
