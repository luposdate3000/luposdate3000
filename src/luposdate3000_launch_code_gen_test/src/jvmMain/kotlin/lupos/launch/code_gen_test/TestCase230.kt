...
public object TestCase230 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/pp09.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/pp09.srx"
    ...
    internal operator fun invoke(){
        println("Test: '(pp09) Reverse sequence path'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp09) Reverse sequence path' success")
        } else {
            println("Result: '(pp09) Reverse sequence path' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp09) Reverse sequence path' failed")
            }
        }
    }
}
