...
public object TestCase143 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/strlang02.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'STRLANG(STR())' success")
        } else {
            println("Result: 'STRLANG(STR())' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'STRLANG(STR())' failed")
            }
        }
    }
}
