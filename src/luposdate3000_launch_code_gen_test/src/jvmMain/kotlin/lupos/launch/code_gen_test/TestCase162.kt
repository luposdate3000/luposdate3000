...
public object TestCase162 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data-builtin-3.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/plus-2.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'plus-2' success")
        } else {
            println("Result: 'plus-2' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'plus-2' failed")
            }
        }
    }
}
