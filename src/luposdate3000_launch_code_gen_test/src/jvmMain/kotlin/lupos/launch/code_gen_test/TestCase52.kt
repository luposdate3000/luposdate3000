...
public object TestCase52 {
    internal const val inputFileName=".//resources/sparql11-test-suite/csv-tsv-res/data2.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/csv-tsv-res/csvtsv03.csv"
    @CodeGenerationAnnotation
    internal const val query = "PREFIX : <http://example.org/> " +
        "SELECT * WHERE { ?s ?p ?o} ORDER BY ?s ?p ?o"
    internal operator fun invoke(){
        println("Test: 'csv03 - CSV Result Format'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'csv03 - CSV Result Format' success")
        } else {
            println("Result: 'csv03 - CSV Result Format' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'csv03 - CSV Result Format' failed")
            }
        }
    }
}
