...
public object TestCase166 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/hash-unicode.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/sha1-02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'SHA1() on Unicode data'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'SHA1() on Unicode data' success")
        } else {
            println("Result: 'SHA1() on Unicode data' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'SHA1() on Unicode data' failed")
            }
        }
    }
}
