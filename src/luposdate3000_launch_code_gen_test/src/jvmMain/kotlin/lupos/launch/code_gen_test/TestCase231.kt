...
public object TestCase231 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/pp10.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/pp10.srx"
    ...
    internal operator fun invoke(){
        println("Test: '(pp10) Path with negation'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp10) Path with negation' success")
        } else {
            println("Result: '(pp10) Path with negation' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp10) Path with negation' failed")
            }
        }
    }
}
