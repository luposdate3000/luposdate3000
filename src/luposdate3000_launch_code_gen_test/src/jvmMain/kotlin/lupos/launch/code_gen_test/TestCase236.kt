...
public object TestCase236 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/data-diamond.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/diamond-2.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: '(pp21) Diamond -- :p+' success")
        } else {
            println("Result: '(pp21) Diamond -- :p+' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp21) Diamond -- :p+' failed")
            }
        }
    }
}
