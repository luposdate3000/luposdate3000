...
public object TestCase148 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/floor01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'FLOOR()' success")
        } else {
            println("Result: 'FLOOR()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'FLOOR()' failed")
            }
        }
    }
}
