...
public object TestCase239 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/data-diamond-loop.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/diamond-loop-5a.srx"
    ...
    internal operator fun invoke(){
        println("Test: '(pp28a) Diamond, with loop -- (:p/:p)?'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp28a) Diamond, with loop -- (:p/:p)?' success")
        } else {
            println("Result: '(pp28a) Diamond, with loop -- (:p/:p)?' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp28a) Diamond, with loop -- (:p/:p)?' failed")
            }
        }
    }
}
