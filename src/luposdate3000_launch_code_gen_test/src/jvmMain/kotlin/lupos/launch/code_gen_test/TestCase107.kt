...
public object TestCase107 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/data-03.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/sparqldl-04.srx"
    @CodeGenerationAnnotation
    internal const val query = "# Query4 " +
    ...
    internal operator fun invoke(){
        println("Test: 'sparqldl-04.rq: bug fixing test'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'sparqldl-04.rq: bug fixing test' success")
        } else {
            println("Result: 'sparqldl-04.rq: bug fixing test' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'sparqldl-04.rq: bug fixing test' failed")
            }
        }
    }
}
