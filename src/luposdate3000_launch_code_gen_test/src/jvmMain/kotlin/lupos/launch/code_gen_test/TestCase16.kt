...
public object TestCase16 {
    internal const val inputFileName=".//resources/sparql11-test-suite/aggregates/agg-numeric.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/aggregates/agg-min-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'MIN' success")
        } else {
            println("Result: 'MIN' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'MIN' failed")
            }
        }
    }
}
