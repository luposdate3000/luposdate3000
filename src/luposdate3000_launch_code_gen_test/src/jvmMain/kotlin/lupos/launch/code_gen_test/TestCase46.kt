...
public object TestCase46 {
    internal const val inputFileName=".//resources/sparql11-test-suite/construct/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/construct/constructwhere02result.ttl"
    @CodeGenerationAnnotation
    internal const val query = "PREFIX : <http://example.org/> " +
        "CONSTRUCT WHERE { :s1 :p ?o . ?s2 :p ?o }"
    internal operator fun invoke(){
        println("Test: 'constructwhere02 - CONSTRUCT WHERE'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'constructwhere02 - CONSTRUCT WHERE' success")
        } else {
            println("Result: 'constructwhere02 - CONSTRUCT WHERE' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'constructwhere02 - CONSTRUCT WHERE' failed")
            }
        }
    }
}
