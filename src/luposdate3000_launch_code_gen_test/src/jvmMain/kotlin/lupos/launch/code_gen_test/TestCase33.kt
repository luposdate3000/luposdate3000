...
public object TestCase33 {
    internal const val inputFileName=".//resources/sparql11-test-suite/bind/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/bind/bind10.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'bind10 - BIND scoping - Variable in filter not in scope'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'bind10 - BIND scoping - Variable in filter not in scope' success")
        } else {
            println("Result: 'bind10 - BIND scoping - Variable in filter not in scope' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'bind10 - BIND scoping - Variable in filter not in scope' failed")
            }
        }
    }
}
