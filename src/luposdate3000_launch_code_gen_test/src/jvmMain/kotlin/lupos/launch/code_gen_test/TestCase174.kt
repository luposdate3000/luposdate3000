...
public object TestCase174 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/month-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'MONTH()' success")
        } else {
            println("Result: 'MONTH()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'MONTH()' failed")
            }
        }
    }
}
