...
public object TestCase250 {
    internal const val inputFileName=".//resources/sparql11-test-suite/subquery/sq10.rdf"
    internal const val targetFileName=".//resources/sparql11-test-suite/subquery/sq10.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'sq10 - Subquery with exists'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'sq10 - Subquery with exists' success")
        } else {
            println("Result: 'sq10 - Subquery with exists' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'sq10 - Subquery with exists' failed")
            }
        }
    }
}
