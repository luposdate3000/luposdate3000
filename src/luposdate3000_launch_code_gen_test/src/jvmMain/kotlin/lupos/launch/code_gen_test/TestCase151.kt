...
public object TestCase151 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data2.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/concat02.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'CONCAT() 2' success")
        } else {
            println("Result: 'CONCAT() 2' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'CONCAT() 2' failed")
            }
        }
    }
}
