...
public object TestCase192 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data4.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/strbefore02.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'STRBEFORE() datatyping'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'STRBEFORE() datatyping' success")
        } else {
            println("Result: 'STRBEFORE() datatyping' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'STRBEFORE() datatyping' failed")
            }
        }
    }
}
