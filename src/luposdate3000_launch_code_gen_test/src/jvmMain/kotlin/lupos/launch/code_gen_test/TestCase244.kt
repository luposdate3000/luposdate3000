...
public object TestCase244 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/clique3.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/pp36.srx"
    @CodeGenerationAnnotation
    internal const val query = "PREFIX : <http://example.org/> " +
        "SELECT * WHERE { :a0 (:p)* :a1 } " +
        ""
    internal operator fun invoke(){
        println("Test: '(pp36) Arbitrary path with bound endpoints'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp36) Arbitrary path with bound endpoints' success")
        } else {
            println("Result: '(pp36) Arbitrary path with bound endpoints' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp36) Arbitrary path with bound endpoints' failed")
            }
        }
    }
}
