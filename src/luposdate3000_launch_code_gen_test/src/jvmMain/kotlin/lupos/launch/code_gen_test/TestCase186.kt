...
public object TestCase186 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/rand01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'RAND()' success")
        } else {
            println("Result: 'RAND()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'RAND()' failed")
            }
        }
    }
}
