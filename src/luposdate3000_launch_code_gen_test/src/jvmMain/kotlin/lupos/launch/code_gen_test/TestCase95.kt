...
public object TestCase95 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/plainLit.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/plainLit.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'Plain literals with language tag are not the same as the same literal without'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'Plain literals with language tag are not the same as the same literal without' success")
        } else {
            println("Result: 'Plain literals with language tag are not the same as the same literal without' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'Plain literals with language tag are not the same as the same literal without' failed")
            }
        }
    }
}
