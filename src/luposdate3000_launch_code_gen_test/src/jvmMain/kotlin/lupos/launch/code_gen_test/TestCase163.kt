...
public object TestCase163 {
    internal const val inputFileName=".//resources/sparql11-test-suite/functions/data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/functions/md5-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'MD5()' success")
        } else {
            println("Result: 'MD5()' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'MD5()' failed")
            }
        }
    }
}
