...
public object TestCase18 {
    internal const val inputFileName=".//resources/sparql11-test-suite/aggregates/agg-numeric.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/aggregates/agg-max-01.srx"
    ...
    internal operator fun invoke(){
        ...
        if (success) {
            println("Result: 'MAX' success")
        } else {
            println("Result: 'MAX' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'MAX' failed")
            }
        }
    }
}
