...
public object TestCase238 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/data-diamond-loop.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/diamond-loop-2.srx"
    ...
    internal operator fun invoke(){
        println("Test: '(pp25) Diamond, with loop -- :p+'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp25) Diamond, with loop -- :p+' success")
        } else {
            println("Result: '(pp25) Diamond, with loop -- :p+' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp25) Diamond, with loop -- :p+' failed")
            }
        }
    }
}
