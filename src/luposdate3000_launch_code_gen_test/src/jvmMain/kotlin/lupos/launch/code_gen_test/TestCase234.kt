...
public object TestCase234 {
    internal const val inputFileName=".//resources/sparql11-test-suite/property-path/pp14.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/property-path/pp14.srx"
    ...
    internal operator fun invoke(){
        println("Test: '(pp14) Star path over foaf:knows'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: '(pp14) Star path over foaf:knows' success")
        } else {
            println("Result: '(pp14) Star path over foaf:knows' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: '(pp14) Star path over foaf:knows' failed")
            }
        }
    }
}
