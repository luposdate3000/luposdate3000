...
public object TestCase102 {
    internal const val inputFileName=".//resources/sparql11-test-suite/entailment/bind-data.ttl"
    internal const val targetFileName=".//resources/sparql11-test-suite/entailment/bind07.srx"
    ...
    internal operator fun invoke(){
        println("Test: 'bind07 - BIND fixed data for OWL DL'")
        var success = true
        try {
        ...
        if (success) {
            println("Result: 'bind07 - BIND fixed data for OWL DL' success")
        } else {
            println("Result: 'bind07 - BIND fixed data for OWL DL' failed")
        }
        if(success){
            var success2 = true
            try {
                ...
                LuposdateEndpoint.close()
            if (!success2) {
                println("ResultCodegen: 'bind07 - BIND fixed data for OWL DL' failed")
            }
        }
    }
}
