...
public class DROPSILENTGRAPHiri {
    val inputData = File("src/jvmTest/resources/DROPSILENTGRAPHiri.input").readAsString()
    val targetData = File("src/jvmTest/resources/DROPSILENTGRAPHiri.output").readAsString()
    val targetType = ".ttl"
    val inputType = ".ttl"
    val query = File("src/jvmTest/resources/DROPSILENTGRAPHiri.query").readAsString()

    @Test fun `DROP SILENT GRAPH iri}`() {
