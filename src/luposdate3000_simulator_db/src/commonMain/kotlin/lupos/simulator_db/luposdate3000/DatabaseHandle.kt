    ...
    private fun receive(pck: MySimulatorAbstractPackage) {
        when (pck.path) {
            ...
            "simulator-intermediate-result" -> {
                SanityCheck.check({/*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/},{ myPendingWorkData[pck.params["key"]!!] == null })
...
//        val nextHops = router!!.getNextDatabaseHops(allHostAdresses)  //TODO
        ...
        while (changed) {
            ...
            loop@ for ((k, v) in pck.dependenciesMapTopDown) {
                if (!packageMap.contains(k)) {
                    SanityCheck.check({/*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/},{ v.size >= 1 })
                    var dest = -1
                    for (key in v) {
        ...
        SanityCheck.check({/*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/},{ packageMap.size == pck.operatorGraph.size })
        for ((k, v) in packages) {
            if (k != ownAdress) {
                router!!.send(k, v)
            }
    ...
    private fun localXMLElementToOPBase(query: Query, node: XMLElement): IOPBase {
        ...
        operatorMap["POPDistributedReceiveSingle"] = { query, node, mapping, recursionFunc ->
            ...
            SanityCheck.check({/*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/},{ keys.size == 1 })
            val key = keys.first()!!
            SanityCheck.check({/*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/},{ myPendingWorkData.contains(key) })
