...
// TODO locking here
    ...
    public fun addPartitionOperator(uuid: Long, id: Int) {
        ...
        if (tmp == null) {
            ...
            SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_operator_base/src/commonMain/kotlin/lupos/operator/base/Query.kt:163"/*SOURCE_FILE_END*/ }, { !tmp.contains(uuid) })
            tmp.add(uuid)
        }
    }
    ...
    public fun removePartitionOperator(uuid: Long, id: Int) {
        val tmp = partitionOperators[id]
        if (tmp != null) {
            SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_operator_base/src/commonMain/kotlin/lupos/operator/base/Query.kt:171"/*SOURCE_FILE_END*/ }, { tmp.contains(uuid) }, { "$uuid $id .. $root" })
            tmp.remove(uuid)
            if (tmp.size == 0) {
