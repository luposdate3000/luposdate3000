...
public class Query public constructor(@JvmField public var dictionary: IDictionary, @JvmField public var transactionID: Long, @JvmField public val instance: Luposdate3000Instance) : IQuery {
    ...
    public fun addPartitionOperator(uuid: Long, id: Int) {
        ...
        if (tmp == null) {
            ...
            SanityCheck.check({ /*SOURCE_FILE_START*/"D:/ideaprojects/luposdate3000/src/luposdate3000_operator_base/src/commonMain/kotlin/lupos/operator/base/Query.kt:135"/*SOURCE_FILE_END*/ }, { !tmp.contains(uuid) })
            tmp.add(uuid)
        }
    }
    ...
    public fun removePartitionOperator(uuid: Long, id: Int) {
        val tmp = partitionOperators[id]
        if (tmp != null) {
            SanityCheck.check({ /*SOURCE_FILE_START*/"D:/ideaprojects/luposdate3000/src/luposdate3000_operator_base/src/commonMain/kotlin/lupos/operator/base/Query.kt:143"/*SOURCE_FILE_END*/ }, { tmp.contains(uuid) })
            tmp.remove(uuid)
            if (tmp.size == 0) {
