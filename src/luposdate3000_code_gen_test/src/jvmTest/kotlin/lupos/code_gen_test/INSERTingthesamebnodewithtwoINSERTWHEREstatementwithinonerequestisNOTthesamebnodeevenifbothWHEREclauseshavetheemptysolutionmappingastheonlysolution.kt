...
public class INSERTingthesamebnodewithtwoINSERTWHEREstatementwithinonerequestisNOTthesamebnodeevenifbothWHEREclauseshavetheemptysolutionmappingastheonlysolution {
    ...
    @Ignore // Reason: >Bug<
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution`() {
    ...
    @Ignore // Reason: >Bug<
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - true - Centralized - true - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - true - Centralized - true - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - true - Centralized - false - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - true - Centralized - false - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - true - Routing - true - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - true - Routing - true - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - true - Routing - false - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - true - Routing - false - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - false - Centralized - true - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - false - Centralized - true - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - false - Centralized - false - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - false - Centralized - false - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - false - Routing - true - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - false - Routing - true - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - false - Routing - false - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByIDTwiceAllCollations - false - Routing - false - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - true - Centralized - true - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - true - Centralized - true - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - true - Centralized - false - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - true - Centralized - false - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - true - Routing - true - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - true - Routing - true - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - true - Routing - false - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - true - Routing - false - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - false - Centralized - true - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - false - Centralized - true - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - false - Centralized - false - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - false - Centralized - false - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - false - Routing - true - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - false - Routing - true - false`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - false - Routing - false - true`() {
    ...
    @Test
    public fun `INSERTing the same bnode with two INSERT WHERE statement within one request is NOT the same bnode even if both WHERE clauses have the empty solution mapping as the only solution - in simulator - PartitionByKeyAllCollations - false - Routing - false - false`() {
