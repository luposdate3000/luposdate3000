...
class MyMapIntLongBTree(val t: Int) {
    ...
    open class MyMapIntLongBTreeNodeIterator(val node: MyMapIntLongBTreeNode?) : Iterator<Int> {
        var i = 0
    ...
    class MyMapIntLongBTreeNode(val t: Int, val leaf: Boolean) {
        ...
        fun iterator() = MyMapIntLongBTreeNodeIterator(this)
        ...
        fun insertNonFull(k: Int, onCreate: () -> Long, onExists: (Int, Long) -> Long) {
            var i = n - 1
            var found = false
            for (j in 0 until n) {
                if (keys[j] as Int == k) {
        ...
        fun splitChild(i: Int, y: MyMapIntLongBTreeNode) {
            val z = MyMapIntLongBTreeNode(y.t, y.leaf)
            z.n = t - 1
    ...
    class MyMapIntLongBTreeInitializer(val t: Int, val target: MyMapIntLongBTree) {
        ...
        fun appendAssumeSorted(key: Int, value: Long): Long {
            ...
            if (data.size == 0 || data[data.size - 1].n == 2 * t - 1) {
                tmp = MyMapIntLongBTreeNode(t, true)
                data.add(tmp)
                tmp.keys[0] = key
                tmp.values[0] = value
                tmp.n = 1
        ...
        fun apply() {
            ...
            while (listA.size > 1) {
                ...
                for (i in 0 until n2 - 1) {
                    val node = MyMapIntLongBTreeNode(t, false)
                    listB.add(node)
                    for (j in 0 until n3) {
                        if (listA.size > 0) {
    ...
    fun insert(k: Int, onCreate: () -> Long, onExists: (Int, Long) -> Long) {
        if (root == null) {
            root = MyMapIntLongBTreeNode(t, true)
            root!!.keys[0] = k
            root!!.values[0] = onCreate()
            root!!.n = 1
            size++
        } else if (root!!.n == 2 * t - 1) {
            val s = MyMapIntLongBTreeNode(t, false)
            s.C[0] = root
            s.splitChild(0, root!!)
            var i = 0
    ...
    class EmptyIterator : MyMapIntLongBTreeNodeIterator(null) {
