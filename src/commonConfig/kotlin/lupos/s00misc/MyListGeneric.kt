...
class MyListGeneric<Generic> {
    ...
    inline fun reserve(capacity: Int) {
        println("Generic<Generic> $uuid reserve $capacity")
    }

    constructor() {
        println("Generic<Generic> $uuid constructor 0")
    }

    constructor(value: Generic) {
        println("Generic<Generic> $uuid constructor 1")
        size = 1
    ...
    constructor(initialCapacity: Int, init: (Int) -> Generic) {
        println("Generic<Generic> $uuid constructor init $initialCapacity")
    ...
    fun clear() {
        println("Generic<Generic> $uuid clear")
        size = 0
        page = MyListGenericPage<Generic>()
        lastpage = page
        println(debug())
    ...
    fun add(value: Generic) {
        println("Generic<Generic> $uuid add")
    ...
    inline operator fun get(idx: Int): Generic {
        println("Generic<Generic> $uuid get $idx")
        require(idx < size)
        var tmp = page
        var offset = 0
    ...
    fun remove(value: Generic): Boolean {
        println("Generic<Generic> $uuid remove $size")
        var i = 0
    ...
    fun removeAt(idx: Int): Generic {
        println("Generic<Generic> $uuid removeAt $idx $size")
        var tmp = page
        var offset = 0
        while (offset + tmp.size < idx) {
            offset += tmp.size
    ...
    inline operator fun set(idx: Int, value: Generic) {
        println("Generic<Generic> $uuid set $idx $size")
        require(idx <= size)
        if (idx == size) {
            if (lastpage.size < lastpage.data.size) {
                lastpage.data[lastpage.size] = value
    ...
    fun add(idx: Int, value: Generic) {
        println("Generic<Generic> $uuid add $idx $size")
        require(idx <= size)
        if (idx == size) {
            if (lastpage.size < lastpage.data.size) {
                println("a")
    ...
    fun debug(): String {
        ...
        var totalsize = 0
        res.append("Generic<Generic> $uuid debug $size [")
        var tmp = page
        while (true) {
            totalsize += tmp.size
            res.append("" + tmp.size + "{")
    ...
    class MyListGenericIterator<Generic>(@JvmField val data: MyListGeneric<Generic>) : Iterator<Generic> {
        ...
        override fun next(): Generic {
            println("Generic<Generic> ${data.uuid} iterator $idx $globalidx")
            if (idx == tmp.size) {
                globalidx += idx
                tmp = tmp.next!!
                idx = 0
