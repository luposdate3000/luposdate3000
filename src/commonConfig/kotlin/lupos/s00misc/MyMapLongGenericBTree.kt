...
class MyMapLongGenericBTree<GenericV>(val t: Int) {
    ...
    open class MyMapLongGenericBTreeNodeIterator<GenericV>(val node: MyMapLongGenericBTreeNode<GenericV>?) : Iterator<Long> {
        var i = 0
    ...
    class MyMapLongGenericBTreeNode<GenericV>(val t: Int, val leaf: Boolean) {
        ...
        fun iterator() = MyMapLongGenericBTreeNodeIterator<GenericV>(this)
        ...
        fun insertNonFull(k: Long, onCreate: () -> GenericV, onExists: (Long, GenericV) -> GenericV) {
            var i = n - 1
            var found = false
            for (j in 0 until n) {
                if (keys[j] as Long == k) {
        ...
        fun splitChild(i: Int, y: MyMapLongGenericBTreeNode<GenericV>) {
            val z = MyMapLongGenericBTreeNode<GenericV>(y.t, y.leaf)
            z.n = t - 1
    ...
    class MyMapLongGenericBTreeInitializer<GenericV>(val t: Int, val target: MyMapLongGenericBTree<GenericV>) {
        ...
        fun appendAssumeSorted(key: Long, value: GenericV): GenericV {
            ...
            if (data.size == 0 || data[data.size - 1].n == 2 * t - 1) {
                tmp = MyMapLongGenericBTreeNode<GenericV>(t, true)
                data.add(tmp)
                tmp.keys[0] = key
                tmp.values[0] = value
                tmp.n = 1
        ...
        fun apply() {
            ...
            while (listA.size > 1) {
                ...
                for (i in 0 until n2 - 1) {
                    val node = MyMapLongGenericBTreeNode<GenericV>(t, false)
                    listB.add(node)
                    for (j in 0 until n3) {
                        if (listA.size > 0) {
    ...
    fun insert(k: Long, onCreate: () -> GenericV, onExists: (Long, GenericV) -> GenericV) {
        if (root == null) {
            root = MyMapLongGenericBTreeNode<GenericV>(t, true)
            root!!.keys[0] = k
            root!!.values[0] = onCreate()
            root!!.n = 1
            size++
        } else if (root!!.n == 2 * t - 1) {
            val s = MyMapLongGenericBTreeNode<GenericV>(t, false)
            s.C[0] = root
            s.splitChild(0, root!!)
            var i = 0
    ...
    class EmptyIterator<GenericV> : MyMapLongGenericBTreeNodeIterator<GenericV>(null) {
