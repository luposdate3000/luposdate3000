...
class MyMapLongDoubleBTree(val t: Int) {
    ...
    open class MyMapLongDoubleBTreeNodeIterator(val node: MyMapLongDoubleBTreeNode?) : Iterator<Long> {
        var i = 0
    ...
    class MyMapLongDoubleBTreeNode(val t: Int, val leaf: Boolean) {
        ...
        fun iterator() = MyMapLongDoubleBTreeNodeIterator(this)
        ...
        fun insertNonFull(k: Long, onCreate: () -> Double, onExists: (Long, Double) -> Double) {
            var i = n - 1
            var found = false
            for (j in 0 until n) {
                if (keys[j] as Long == k) {
        ...
        fun splitChild(i: Int, y: MyMapLongDoubleBTreeNode) {
            val z = MyMapLongDoubleBTreeNode(y.t, y.leaf)
            z.n = t - 1
    ...
    class MyMapLongDoubleBTreeInitializer(val t: Int, val target: MyMapLongDoubleBTree) {
        ...
        fun appendAssumeSorted(key: Long, value: Double): Double {
            ...
            if (data.size == 0 || data[data.size - 1].n == 2 * t - 1) {
                tmp = MyMapLongDoubleBTreeNode(t, true)
                data.add(tmp)
                tmp.keys[0] = key
                tmp.values[0] = value
                tmp.n = 1
        ...
        fun apply() {
            ...
            while (listA.size > 1) {
                ...
                for (i in 0 until n2 - 1) {
                    val node = MyMapLongDoubleBTreeNode(t, false)
                    listB.add(node)
                    for (j in 0 until n3) {
                        if (listA.size > 0) {
    ...
    fun insert(k: Long, onCreate: () -> Double, onExists: (Long, Double) -> Double) {
        if (root == null) {
            root = MyMapLongDoubleBTreeNode(t, true)
            root!!.keys[0] = k
            root!!.values[0] = onCreate()
            root!!.n = 1
            size++
        } else if (root!!.n == 2 * t - 1) {
            val s = MyMapLongDoubleBTreeNode(t, false)
            s.C[0] = root
            s.splitChild(0, root!!)
            var i = 0
    ...
    class EmptyIterator : MyMapLongDoubleBTreeNodeIterator(null) {
