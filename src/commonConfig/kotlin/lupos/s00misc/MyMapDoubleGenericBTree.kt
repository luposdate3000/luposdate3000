...
class MyMapDoubleGenericBTree<GenericV>(val t: Int) {
    ...
    open class MyMapDoubleGenericBTreeNodeIterator<GenericV>(val node: MyMapDoubleGenericBTreeNode<GenericV>?) : Iterator<Double> {
        var i = 0
    ...
    class MyMapDoubleGenericBTreeNode<GenericV>(val t: Int, val leaf: Boolean) {
        ...
        fun iterator() = MyMapDoubleGenericBTreeNodeIterator<GenericV>(this)
        ...
        fun insertNonFull(k: Double, onCreate: () -> GenericV, onExists: (Double, GenericV) -> GenericV) {
            var i = n - 1
            var found = false
            for (j in 0 until n) {
        ...
        fun splitChild(i: Int, y: MyMapDoubleGenericBTreeNode<GenericV>) {
            val z = MyMapDoubleGenericBTreeNode<GenericV>(y.t, y.leaf)
            z.n = t - 1
    ...
    class MyMapDoubleGenericBTreeInitializer<GenericV>(val t: Int, val target: MyMapDoubleGenericBTree<GenericV>) {
        ...
        fun appendAssumeSorted(key: Double, value: GenericV): GenericV {
            ...
            if (data.size == 0 || data[data.size - 1].n == 2 * t - 1) {
                tmp = MyMapDoubleGenericBTreeNode<GenericV>(t, true)
                data.add(tmp)
                tmp.keys[0] = key
                tmp.values[0] = value
                tmp.n = 1
        ...
        fun apply() {
            ...
            while (listA.size > 1) {
                ...
                for (i in 0 until n2 - 1) {
                    val node = MyMapDoubleGenericBTreeNode<GenericV>(t, false)
                    listB.add(node)
                    for (j in 0 until n3) {
                        if (listA.size > 0) {
    ...
    fun insert(k: Double, onCreate: () -> GenericV, onExists: (Double, GenericV) -> GenericV) {
        if (root == null) {
            root = MyMapDoubleGenericBTreeNode<GenericV>(t, true)
            root!!.keys[0] = k
            root!!.values[0] = onCreate()
            root!!.n = 1
            size++
        } else if (root!!.n == 2 * t - 1) {
            val s = MyMapDoubleGenericBTreeNode<GenericV>(t, false)
            s.C[0] = root
            s.splitChild(0, root!!)
            var i = 0
    ...
    class EmptyIterator<GenericV> : MyMapDoubleGenericBTreeNodeIterator<GenericV>(null) {
