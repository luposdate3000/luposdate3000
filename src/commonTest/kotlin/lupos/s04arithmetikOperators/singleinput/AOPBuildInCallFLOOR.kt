...
class AOPBuildInCallFLOORTest {

    @TestFactory
    fun testCalculate() = helperTest.forEachNumeric { x, ad, at ->
        val expected = helperTest.toConstant(floor(ad), at)
        DynamicTest.dynamicTest("calculate(${x.valueToString()} to ${expected.valueToString()})") {
            val resultSet = ResultSet(ResultSetDictionary())
            val output = AOPBuildInCallFLOOR(x).calculate(resultSet, resultSet.createResultRow())
