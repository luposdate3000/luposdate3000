...
public class LoggerMeasure public constructor(private val simRun: SimulationRun) : ILogger {
    public companion object {
        ...
        public const val StatNumberOfQuerySenders: Int = 3
        public const val StatNumberOfLinks: Int = 4
        public const val StatSimulationDurationReal: Int = 5
        public const val StatSimulationDurationVirtual: Int = 6
        public const val StatNumberOfSentPackages: Int = 7
        public const val StatNetworkTrafficInKiloBytes: Int = 8
        ...
        public const val StatCounter: Int = 15
    }
    ...
    public val headers: Array<String> = Array(StatCounter) {
        when (it) {
            ...
            StatNetworkTrafficInKiloBytes -> "network traffic (KiB)"
    ...
    override fun onSendNetworkPackage(src: Int, dest: Int, hop: Int, pck: IPayload, delay: Long) {
        data[StatNetworkTrafficInKiloBytes] += pck.getSizeInBytes().toDouble()
        if (dest != hop) {
    ...
    override fun addConnectionTable(src: Int, dest: Int, hop: Int) {
        data[StatNumberOfLinks] += 0.5 // bidirectional ...
    }
    ...
    override fun onStopSimulation() {
        val shutDownTimeStamp = startUpTimeStamp.plus(simRun.sim.clock, DateTimeUnit.NANOSECOND, TimeZone.UTC)
        val realShutDownTimeStamp = Clock.System.now()
        data[StatSimulationDurationReal] = (realShutDownTimeStamp - startUpTimeStamp).inWholeNanoseconds.toDouble() / 1000000.0
        data[StatSimulationDurationVirtual] = (shutDownTimeStamp - startUpTimeStamp).inWholeNanoseconds.toDouble() / 1000000.0
    }
