            ...
            if (c.getProvidedVariableNames().isEmpty()) {
                ...
                for (s in joinColumns) {
                    if (keepOrder) {
                        ...
                        c.mySortPriority = sortPriority.filter { c.getProvidedVariableNames().contains(it.variableName) }.toMutableList()
                        query.addPartitionOperator(c.uuid, newID[i])
                    } else {
                        ...
                        if (flag) {
                            res = if (childA.getProvidedVariableNames().containsAll(node.getMySortPriority().map { it.variableName })) {
                                if (node.optional) {
                                    ...
                                    embedWithinPartitionContext(columns[0], childA, childB, { a, b -> POPJoinMerge(query, projectedVariables, a, b, false) }, true, node.getMySortPriority())
                                }
                            } else {
                                if (node.optional) {
                                    embedWithinPartitionContext(columns[0], childA, childB, { a, b -> POPJoinMergeOptional(query, projectedVariables, a, b, true) }, true, node.getMySortPriority())
                                } else {
                                    embedWithinPartitionContext(columns[0], childB, childA, { a, b -> POPJoinMerge(query, projectedVariables, a, b, false) }, true, node.getMySortPriority())
                                }
                            }
                        }
                    }
