...
class NodeLeafColumnIterator1(node: ByteArray, nodeid: Int, lock: ReadWriteLock) : NodeLeafColumnIterator(node, nodeid, lock) {
    ...
    suspend override fun next(): Int {
        value = next_helper(value, NodeShared::readTriple010)
        return value
    }

    suspend override fun nextSIP(minValue: Int, crossinline skippedElements: (counter: Int) -> Unit): Int {
        value = nextSIP_helper(value, minValue, skippedElements, NodeShared::readTriple010)
        println("usedSIPMinValue")
        return value
    }

    override suspend open fun nextSIP(skipCount: Int): Int {
        value = nextSIP_helper(value, skipCount, NodeShared::readTriple010)
        println("usedSIPSkip")
        return value
    }
}
