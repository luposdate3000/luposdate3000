...
internal object NodeShared {
        ...
        SanityCheck(
            { /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ },
            {
                decodeTripleHeader(header) { c0, c1, c2 ->
                    SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { c0 == counter0 })
                    SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { c1 == counter1 })
                    SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { c2 == counter2 })
                }
            }
        )
    }
    ...
    internal inline fun writeTriple(node: ByteArray, offset: Int, l: DictionaryValueTypeArray, d: DictionaryValueTypeArray): Int {
        ...
        val b1 = l[1] xor d[1]
        val b2 = l[2] xor d[2]
        SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { d[0] >= 0 })
        SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { d[1] >= 0 })
        SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { d[2] >= 0 })
        SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { l[0] >= 0 })
        SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { l[1] >= 0 })
        SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { l[2] >= 0 })
        SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { b0 >= 0 })
        SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { b1 >= 0 })
        SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { b2 >= 0 })
        ...
        SanityCheck(
            { /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ },
            {
                ...
                SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { size == localOff - offset })
                size = readTriple100(node, offset, l[0]) { n0 ->
                    SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { n0 == d[0] })
                }
                SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { size == localOff - offset })
                size = readTriple010(node, offset, l[1]) { n1 ->
                    SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { n1 == d[1] })
                }
                SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { size == localOff - offset })
                size = readTriple001(node, offset, l[2]) { n2 ->
                    SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { n2 == d[2] })
                }
                SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { size == localOff - offset })
