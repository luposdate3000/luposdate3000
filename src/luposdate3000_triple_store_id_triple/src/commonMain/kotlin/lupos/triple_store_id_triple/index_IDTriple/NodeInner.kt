...
internal object NodeInner {
    ...
    internal inline fun initializeWith(node: ByteArray, childs: MutableList<Int>, nodeManager: NodeManager) {
        SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { childs.size > 0 })
        ...
        SanityCheck(
            { /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ },
            {
                ...
                var lastChildPointer = getFirstChild(node)
                SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { lastChildPointer == writtenHeaders!![0] })
                var i = 0
                var value0: DictionaryValueType = 0
                var value1: DictionaryValueType = 0
                var value2: DictionaryValueType = 0
                while (remaining > 0) {
                    ...
                    SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { lastChildPointer == writtenHeaders!![i + 1] })
                    remaining--
                    i++
                }
            }
