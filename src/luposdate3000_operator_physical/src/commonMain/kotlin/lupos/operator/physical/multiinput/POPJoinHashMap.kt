...
public class POPJoinHashMap public constructor(
    ...
    override fun getPartitionCount(variable: String): Int {
        return if (children[0].getProvidedVariableNames().contains(variable)) {
            if (children[1].getProvidedVariableNames().contains(variable)) {
                SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_operator_physical/src/commonMain/kotlin/lupos/operator/physical/multiinput/POPJoinHashMap.kt:48"/*SOURCE_FILE_END*/ }, { children[0].getPartitionCount(variable) == children[1].getPartitionCount(variable) })
    ...
    override fun toLocalOperatorGraph(parent: Partition, onFoundLimit: (IPOPLimit) -> Unit, onFoundSort: () -> Unit): POPBase? {
        ...
        if (tmp1 == null) {
println("deny POPJoinHashMap a")
            return null
        }
        val tmp2 = (children[1]as POPBase).toLocalOperatorGraph(parent, onFoundLimit, onFoundSort)
        if (tmp2 == null) {
println("deny POPJoinHashMap b")
            return null
        }
        return POPJoinHashMap(query, projectedVariables, tmp1, tmp2, optional)
    }
}
