...
public class POPValuesImportXML public constructor(query: IQuery, projectedVariables: List<String>, data: XMLElement) : POPValuesImportBase(query, projectedVariables, data["head"]!!.childs.map { it.attributes["name"]!! }) {
    init {
        ...
        SanityCheck.check({ /*SOURCE_FILE_START*/"D:/ideaprojects/luposdate3000/src/luposdate3000_operator_physical/src/commonMain/kotlin/lupos/operator/physical/noinput/POPValuesImportXML.kt:25"/*SOURCE_FILE_END*/ }, { data.tag == "sparql" })
        for (node in data["results"]!!.childs) {
            ...
            for (v in node.childs) {
                ...
                SanityCheck.check({ /*SOURCE_FILE_START*/"D:/ideaprojects/luposdate3000/src/luposdate3000_operator_physical/src/commonMain/kotlin/lupos/operator/physical/noinput/POPValuesImportXML.kt:34"/*SOURCE_FILE_END*/ }, { !((datatype != null) && (lang != null)) })
                when {
                    child.tag == "uri" -> {
