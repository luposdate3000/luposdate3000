...
public object LogicalOptimizerJoinOrderCostBasedOnVariable {
    ...
    public /*suspend*/ operator fun invoke(allChilds: List<IOPBase>, root: LOPJoin): IOPBase? {
        SanityCheck.check({ /*SOURCE_FILE_START*/"D:/ideaprojects/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/LogicalOptimizerJoinOrderCostBasedOnVariable.kt:42"/*SOURCE_FILE_END*/ }, { allChilds.size > 2 })
        if (allChilds.size < 30) {
            ...
            for (i in allChilds.indices) {
                ...
                val tmp = allChilds[i].getProvidedVariableNames()
                for (t in tmp) {
                    SanityCheck.check({ /*SOURCE_FILE_START*/"D:/ideaprojects/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/LogicalOptimizerJoinOrderCostBasedOnVariable.kt:77"/*SOURCE_FILE_END*/ }, { allVariables.contains(t) })
                    for (j in allVariables.indices) {
                        if (allVariables[j] == t) {
                            variables[j]++
                            break
