    ...
    private fun verifyPartitionOperators(node: IOPBase, allList: MutableMap<Int, MutableSet<Long>>, currentPartitions_: MutableMap<String, Int>, root: IOPBase) {
        ...
        when (node) {
            ...
            is POPMergePartition -> {
                SanityCheck.check(
                    { /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/OptimizerCompoundBase.kt:44"/*SOURCE_FILE_END*/ },
            ...
            is POPMergePartitionOrderedByIntId -> {
                SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/OptimizerCompoundBase.kt:52"/*SOURCE_FILE_END*/ }, { !currentPartitions.contains(node.partitionVariable) })
                currentPartitions[node.partitionVariable] = node.partitionCount
                ids.add(node.partitionID)
            }
            is POPSplitPartitionFromStore -> {
                SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/OptimizerCompoundBase.kt:57"/*SOURCE_FILE_END*/ }, { currentPartitions[node.partitionVariable] == node.partitionCount }, { root.toXMLElement(false).toPrettyString() })
                currentPartitions[node.partitionVariable] = -node.partitionCount
                ids.add(node.partitionID)
            }
            is POPSplitPartitionFromStoreCount -> {
                SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/OptimizerCompoundBase.kt:62"/*SOURCE_FILE_END*/ }, { currentPartitions[node.partitionVariable] == node.partitionCount }, { root.toXMLElement(false).toPrettyString() })
                currentPartitions[node.partitionVariable] = -node.partitionCount
                ids.add(node.partitionID)
            }
            is POPSplitPartition -> {
                SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/OptimizerCompoundBase.kt:67"/*SOURCE_FILE_END*/ }, { currentPartitions[node.partitionVariable] == node.partitionCount }, { "$root" })
                currentPartitions.remove(node.partitionVariable)
                ids.add(node.partitionID)
            }
            is POPChangePartitionOrderedByIntId -> {
                SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/OptimizerCompoundBase.kt:72"/*SOURCE_FILE_END*/ }, { currentPartitions[node.partitionVariable] == node.partitionCountTo }, { "$root" })
