    ...
    private fun verifyPartitionOperators(node: IOPBase, allList: MutableMap<Int, MutableSet<Long>>, currentPartitions_: MutableMap<String, Int>, root: IOPBase) {
        ...
        when (node) {
            is POPMergePartitionCount -> {
                SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/OptimizerCompoundBase.kt:38"/*SOURCE_FILE_END*/ }, { !currentPartitions.contains(node.partitionVariable) })
                currentPartitions[node.partitionVariable] = node.partitionCount
                ids.add(node.partitionID)
            }
            is POPMergePartition -> {
                SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/OptimizerCompoundBase.kt:43"/*SOURCE_FILE_END*/ }, { !currentPartitions.contains(node.partitionVariable) })
                currentPartitions[node.partitionVariable] = node.partitionCount
                ids.add(node.partitionID)
            }
            is POPMergePartitionOrderedByIntId -> {
                SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/OptimizerCompoundBase.kt:48"/*SOURCE_FILE_END*/ }, { !currentPartitions.contains(node.partitionVariable) })
                currentPartitions[node.partitionVariable] = node.partitionCount
                ids.add(node.partitionID)
            }
            ...
            is POPSplitPartition -> {
                SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/OptimizerCompoundBase.kt:63"/*SOURCE_FILE_END*/ }, { currentPartitions[node.partitionVariable] == node.partitionCount }, { "$root" })
                currentPartitions.remove(node.partitionVariable)
                ids.add(node.partitionID)
            }
            is POPChangePartitionOrderedByIntId -> {
                SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/OptimizerCompoundBase.kt:68"/*SOURCE_FILE_END*/ }, { currentPartitions[node.partitionVariable] == node.partitionCountTo }, { "$root" })
        ...
        for (opt in childrenOptimizers) {
            ...
            while (d) {
                ...
                for (o in opt) {
                    ...
                    SanityCheck(
                        { /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/OptimizerCompoundBase.kt:111"/*SOURCE_FILE_END*/ },
                        {
                            ...
                            for ((k, v1) in allPartitionOperators) {
                                ...
                                SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/OptimizerCompoundBase.kt:117"/*SOURCE_FILE_END*/ }, { v1 == v2 }, { "$allPartitionOperators  <-a-> ${query.partitionOperators}\n$tmp" })
                            }
                            for ((k, v1) in query.partitionOperators) {
                                val v2 = allPartitionOperators[k]
                                SanityCheck.check({ /*SOURCE_FILE_START*/"/src/luposdate3000/src/luposdate3000_optimizer_logical/src/commonMain/kotlin/lupos/optimizer/logical/OptimizerCompoundBase.kt:121"/*SOURCE_FILE_END*/ }, { v1 == v2 }, { "$allPartitionOperators  <-b-> ${query.partitionOperators}\n$tmp" })
                            }
                            if (query.filtersMovedUpFromOptionals) {
