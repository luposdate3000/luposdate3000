...
public class TripleStoreManagerImpl : TripleStoreManager {
    ...
    private inline fun metadataRemove(name: LuposGraphName) {
        SanityCheck.check({/*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/},{ metadata_[name] != null || name == DEFAULT_GRAPH_NAME }, { "$name :: ${metadata_.keys}" })
    ...
    private inline fun initFromPageID() {
        ...
        var page = bufferManager.getPage(/*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/, pageid)
    ...
    private inline fun deleteAllPagesExceptRootID() {
        ...
        while (off < size) {
            ...
            val len2 = min(size - off, BufferManagerPage.BUFFER_MANAGER_PAGE_SIZE_IN_BYTES - 4)
            off += len2
            bufferManager.deletePage(/*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/, pageid)
        }
    }
    ...
    private inline fun writeToPageID() {
        ...
        off += len
        while (off < size) {
            val pageid2 = bufferManager.allocPage(/*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/)
            BufferManagerPage.writeInt4(page, 0, pageid2)
            bufferManager.releasePage(/*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/, pageid)
            pageid = pageid2
            page = bufferManager.getPage(/*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/, pageid2)
            val len2 = min(size - off, BufferManagerPage.BUFFER_MANAGER_PAGE_SIZE_IN_BYTES - 4)
    ...
    override fun delete() {
        ...
        deleteAllPagesExceptRootID()
        bufferManager.getPage(/*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/, rootPageID)
        ...
        for (index in graph.indices) {
            for (store in index.getAllLocations()) {
                if (store.first == localhost) {
                    val page = bufferManager.allocPage(/*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/)
