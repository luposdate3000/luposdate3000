...
for i in range(1,18):
 ...
 cursor.execute("select SUM(ctr) as sum_dist_from_avg_1000 from( select count(*) as ctr from benchmark_values bv2 join (select bv1.dataset_id,bv1.query_id,AVG(bv1.value/bv1.valuetime) AS a,COUNT(*) as c from benchmark_values bv1 where bv1.value is not null and bv1.valuetime is not null and bv1.valuetime>=%s and bv1.valuetime<=%s group by bv1.dataset_id,bv1.query_id) as bv3 on bv3.dataset_id=bv2.dataset_id and bv3.query_id = bv2.query_id where (((bv2.value/bv2.valuetime)/a>1.2) OR (a/(bv2.value/bv2.valuetime))>1.2) and c >1 group by bv3.dataset_id,bv3.query_id )as bv4;",(low,high))
 value_actual = cursor.fetchone()[0]

 cursor.execute("select SUM(ctr) as sum_all_for_avg_1000 from( select count(*) as ctr from benchmark_values bv2 join (select bv1.dataset_id,bv1.query_id,COUNT(*) as c from benchmark_values bv1 where bv1.value is not null and bv1.valuetime is not null and bv1.valuetime>=%s and bv1.valuetime<=%s group by bv1.dataset_id,bv1.query_id) as bv3 on bv3.dataset_id=bv2.dataset_id and bv3.query_id = bv2.query_id where c >1 group by bv3.dataset_id,bv3.query_id)as bv4;",(low,high))
 value_max = cursor.fetchone()[0]

 data=None
 if value_actual is not None:
  data=value_actual/value_max
 print(low,high,data)
