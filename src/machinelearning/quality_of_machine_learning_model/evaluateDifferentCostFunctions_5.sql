...
create table evaluate_cross_score_based_on_intermediate as select oc.optimizer_id, oc.query_id, ((case when bv.value<0 then bvmm.mmax*2 else bv.value end) - bvmm.mmin)/(bvmm.mmax*2-bvmm.mmin)*2 as intermediate_based, ((case when bvn.value<0 then bvnmm.mmax*2 else bvn.value end) - bvnmm.mmin)/(bvnmm.mmax*2-bvnmm.mmin)*2 as traffic_based from optimizer_choice oc, benchmark_values bv, benchmark_values_network_traffic bvn, evaluate_benchmark_values_minmax bvmm, evaluate_benchmark_values_network_traffic_minmax bvnmm, mapping_query mq where oc.query_id=bv.query_id and oc.join_id =bv.join_id and oc.query_id=bvn.query_id and oc.join_id=bvn.join_id and oc.query_id=bvmm.query_id and oc.query_id=bvnmm.query_id and mq.id=oc.query_id and mq.rng>0.7 and mq.triplepatterns=5;
...
% select * from evaluate_ranking_based_on_network_traffic n, evaluate_ranking_based_on_intermediate i, mapping_optimizer o where o.id=n.optimizer_id and n.optimizer_id=i.optimizer_id and name != "all" and (name not like "%model" or name like "%_1048576.model") order by intermediate_on_traffic;
% select * from evaluate_ranking_based_on_network_traffic n, evaluate_ranking_based_on_intermediate i, mapping_optimizer o where o.id=n.optimizer_id and n.optimizer_id=i.optimizer_id and name != "all" and (name not like "%model" or name like "%_1048576.model") order by traffic_on_traffic;
% select * from evaluate_ranking_based_on_network_traffic n, evaluate_ranking_based_on_intermediate i, mapping_optimizer o where o.id=n.optimizer_id and n.optimizer_id=i.optimizer_id and name != "all" and (name not like "%model" or name like "%_1048576.model") order by intermediate_on_intermediate;
% select * from evaluate_ranking_based_on_network_traffic n, evaluate_ranking_based_on_intermediate i, mapping_optimizer o where o.id=n.optimizer_id and n.optimizer_id=i.optimizer_id and name != "all" and (name not like "%model" or name like "%_1048576.model") order by traffic_on_intermediate;

% percentage of too much data or too much intermediates
