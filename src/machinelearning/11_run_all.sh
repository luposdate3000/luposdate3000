...
do
ii=$(printf "%04d" $i)
echo "./12_joinopti_agent_eval.py 1 5 20 0.7 /src/luposdate3000/src/machinelearning/_tmpdata/complete.n3.nt  _tmpdata/model/model_${arr[0]}_${arr[1]}_20_0.7_${y}.model" > tasks/$ii.sh
echo "retVal=\$?" >> tasks/$ii.sh
echo "if [ \$retVal -eq 0 ]; then" >> tasks/$ii.sh
echo "truncate -s0 tasks/$ii.sh" >> tasks/$ii.sh
echo "fi" >> tasks/$ii.sh
