...
for x in 3_3 4_4 5_5 3_4 3_5 4_5
do
arr=(${x//_/ })
ii=$(printf "%04d" $i)
echo "./11_joinopti_agent_train.py ${arr[0]} ${arr[1]} 20 0.7 /src/luposdate3000/src/machinelearning/_tmpdata/complete.n3.nt /src/luposdate3000/src/machinelearning/models/" > tasks/$ii.sh
...
do
ii=$(printf "%04d" $i)
echo "./12_joinopti_agent_eval.py 1 5 20 0.7 /src/luposdate3000/src/machinelearning/_tmpdata/complete.n3.nt models/model_${arr[0]}_${arr[1]}_20_0.7_${y}.model" > tasks/$ii.sh
echo "retVal=\$?" >> tasks/$ii.sh
echo "if [ \$retVal -eq 0 ]; then" >> tasks/$ii.sh
echo "truncate -s0 tasks/$ii.sh" >> tasks/$ii.sh
echo "fi" >> tasks/$ii.sh
