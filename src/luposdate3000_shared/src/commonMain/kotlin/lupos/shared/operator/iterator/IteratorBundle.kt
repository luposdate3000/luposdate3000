...
public open class IteratorBundle {
        ...
        get() {
            return when (mode) {
                IteratorBundleModeExt.COLUMN -> {
                    SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { _columns!!.isNotEmpty() })
                    _columns!!
                }
    ...
    /*suspend*/ public fun count(): Int {
        SanityCheck.check({ /*SOURCE_FILE_START*/""/*SOURCE_FILE_END*/ }, { mode == IteratorBundleModeExt.COUNT })
        return if (counter > 0) {
            counter
        } else {
            var res = 0
